...
The server then keeps track of the current CDC stream position on the database (LSN in Postgres, resume token in MongoDB, GTID + Binlog Position in MySQL, or CDC LSN in SQL Server), and notifies the client when the data has been replicated, as part of checkpoint data in the normal data stream.
