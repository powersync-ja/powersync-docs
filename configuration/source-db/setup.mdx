...
import PostgresRdsPowerSyncUser from '/snippets/postgres-rds-powersync-user.mdx';
...
<AccordionGroup>
  ...
  <Accordion title="AWS Aurora">
    PowerSync replicates from the writer, the primary instance of an Aurora Postgres cluster. This applies to provisioned clusters and to Aurora Serverless v2. All currently available Aurora Postgres versions support logical replication.

    ### Prerequisites

    PowerSync must be able to reach the writer. Either make the cluster publicly accessible over IPv4 and restrict access to PowerSync's IPs (see [IP Filtering](/configuration/source-db/security-and-ip-filtering)), or connect through a [Private Endpoint](/configuration/source-db/private-endpoints) using AWS PrivateLink.

    ### 1. Ensure logical replication is enabled

    In Aurora, `rds.logical_replication` is a setting of the **DB cluster parameter group**, not of a DB instance parameter group.

    1. In the [Amazon RDS console](https://console.aws.amazon.com/rds/), open your cluster's **Configuration** tab and follow the **Parameter group** link of type **DB cluster parameter group**. If the cluster still uses a default parameter group, [create a custom DB cluster parameter group](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_WorkingWithDBClusterParamGroups.html) and assign it to the cluster first. Default parameter groups cannot be edited.
    2. Set `rds.logical_replication` to `1` and save.
    3. Reboot the writer. The change only takes effect after the reboot.
    4. Connect to the writer through the cluster endpoint and confirm the settings:
...
The permissions PowerSync requires depend on whether you use MongoDB Atlas built-in roles, or a custom role on either self-hosted MongoDB or Atlas.
...
#### Privileges Required: Self-Hosted or Custom Roles
...
<CardGroup cols="2">
  <Card title="PowerSync Cloud:">
    ...
    and instance and go to **Database Connection** to edit the connection settings.
  </Card>
  <Card
...
- **Do not modify the internal checkpoints collection.** PowerSync maintains a `_powersync_checkpoints` collection in the source database for replication bookkeeping. Dropping it or deleting its documents disrupts replication.
...
- Schema changes are not adopted automatically. Adopting a schema change requires deploying an updated Sync Config. See [SQL Server Specifics](/maintenance-ops/implementing-schema-changes#sql-server-specifics) for the recommended workflows.
...
- Wildcard table names (`%`) are not supported. Every replicated table must be listed by name in your Sync Streams.
...
Every table referenced in your Sync Streams must exist and have CDC enabled before replication can start.
...
<Warning>
  ...
  We also can't yet guarantee continued support or long-term stability. This release is intended for early testing
  and to invite feedback. Your feedback will directly influence whether, and how, this integration evolves.
</Warning>
...
- A Convex deploy key. In the Convex Dashboard, go to **Settings** → **General** and generate a deploy key for the deployment PowerSync should replicate. Create the key with **Custom permissions** and include `deployment:data:view`; the **Deploy only** option does not work for PowerSync replication.
- The `powersync_checkpoints` table and `createCheckpoint` mutation described below.

<Warning>
  Use a deploy key for the correct environment, grant only the permissions PowerSync needs, store it as a secret, and
...
Convex API requests time out after 60000ms by default. When self-hosting PowerSync, you can configure this with the optional `request_timeout_ms` connection parameter:
...
Lowering `polling_interval_ms` can reduce replication latency, but it increases the number of requests made to Convex and the work performed by the PowerSync Service. For more on monitoring and reducing replication lag, see [Replication Lag](/maintenance-ops/replication-lag).

### Client ID Mapping

Convex generates its own document IDs and exposes them as `_id`. PowerSync clients need a stable local ID before a write is uploaded, so plan your Convex schema around a client-generated UUID rather than the Convex `_id`.

<Warning>
  Clients cannot create Convex IDs before inserting documents into Convex. Use a client-generated UUID column as the
  synced `id` and keep `_id` as the Convex server-generated document ID. This is similar to the pattern described in
  [Sequential ID Mapping](/client-sdks/advanced/sequential-id-mapping).
</Warning>
...
For a complete [Sync Streams](/sync/streams/overview) configuration that applies this mapping, see [Convex with ID Mapping](/sync/streams/examples#convex-with-id-mapping). For how Convex data types map to SQLite columns, see [Convex Type Mapping](/sync/types#convex-type-mapping).

### Client Writes

Apply queued client-side writes to Convex by calling Convex mutations from your backend connector. See [Writing Client Changes](/handling-writes/writing-client-changes#convex) for the upload path and authentication details.

### Demo App

For a working JavaScript example, see the [PowerSync Convex To-Do List Demo](https://github.com/powersync-community/powersync-convex-todolist-demo).
...
<CardGroup cols={2}>
  ...
  <Card title="Self-Hosted PowerSync" href="/configuration/powersync-service/self-hosted-instances#replication">
  </Card>
</CardGroup>
