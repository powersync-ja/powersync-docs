...
<Note>
  ...
  - We are continuously improving the reporting and tools to help you troubleshoot usage. Please [reach out](/resources/contact-us) if you have any feedback or need help.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="What is data synced?">
    ...
    It measures the total uncompressed size of data synced from PowerSync Service instances to client devices. 
...
3. Compare operations vs row sizes
   - If operations are high and size scales with it, you likely have high-churn tables, new users syncing full histories.
   - If operations are moderate but size is large, your rows are likely wide (big JSON/text/blob columns).
4. Identify large payloads in your DB
   - Check typical row sizes for high-traffic tables and look for large columns (e.g. long TEXT/JSON fields, embedded files).
