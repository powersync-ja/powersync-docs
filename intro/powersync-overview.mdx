...
description: "A sync engine that automatically syncs your backend database to a local SQLite database on the device. Your app reads and writes directly to this database, so it responds instantly and stays fully functional without a network connection."
sidebarTitle: Introduction
---

import ClientSdks from '/snippets/client-sdks.mdx';

PowerSync is made up of the PowerSync Service and a set of client SDKs. The PowerSync Service replicates data from your backend database, partitions it based on what data each user should receive, and streams real-time updates to clients through the PowerSync SDK.
