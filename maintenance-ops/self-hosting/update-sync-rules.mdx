...
Define Sync Streams (or legacy Sync Rules) in your `powersync.yaml` either inline or via a separate file.
...
<Steps>
  <Step title="Edit Config">
    ...
    <CodeGroup>
      ```yaml Sync Streams — Inline (Recommended)
      sync_config:
        content: |
          config:
            edition: 3
      ...
      ```yaml Sync Streams — Separate File
      sync_config:
