...
description: "Deploy the self-hosted PowerSync Service on AWS ECS with Fargate."
---
...
- [Sync Streams](/sync/streams/overview): Define which data to sync to clients
...
- [Monitoring](/maintenance-ops/self-hosting/monitoring): Enable the Prometheus metrics endpoint for connection-based auto-scaling (used in the [Auto Scaling](#auto-scaling-high-availability-setup) section):
  ...
  ```
- [Bucket Storage](/configuration/powersync-service/self-hosted-instances#storage): Connection details for your bucket storage database. PowerSync supports MongoDB or Postgres as bucket storage databases. In this guide, we focus on MongoDB.
  <Tabs>
