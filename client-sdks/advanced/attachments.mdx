...
<Warning>
The `@powersync/attachments` package (JavaScript/TypeScript) and `powersync_attachments_helper` package (Flutter/Dart) are deprecated. Attachment functionality is now built-in to the PowerSync SDKs. Please use the [built-in attachment helpers](#sdk-%26-demo-reference) instead.
</Warning>
...
- **[Supabase Storage Integration](/integrations/supabase/attachments)** - Detailed guide for using Supabase Storage
- **[AWS S3 Storage Adapter Tutorial](/client-sdks/advanced/attachments-aws-s3-storage)** - How to implement an S3 adapter
- **[PDF Attachments](/client-sdks/advanced/attachments-pdfs)** - Complete API reference
