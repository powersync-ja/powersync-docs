...
- Convex `json_schemas` can omit sparse fields until stored data exists for those fields. Because raw Convex `Int64` values look like regular strings without schema metadata, we recommend casting Convex `Int64` fields to `TEXT` in Sync Streams, for example `CAST(an_int64_column AS TEXT) AS an_int64_column`.
