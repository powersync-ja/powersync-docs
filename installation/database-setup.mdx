...
<Check>
  **Version compatibility**: 
  ...
  Database.
  - SQL Server support was introduced in version 1.18.0 of the PowerSync Service.
</Check>
...
- Spatial data types are returned as JSON objects as supplied by the Tedious node mssql client. See the notes for node-mssql driver [here](https://github.com/tediousjs/node-mssql?tab=readme-ov-file#geography-and-geometry).
...
GRANT SELECT ON <YOUR_SCHEMA>.<YOUR_TABLE_NAME> TO [powersync_user];
...
```sql
GRANT SELECT, INSERT, UPDATE ON <YOUR_SCHEMA>._powersync_checkpoints TO [powersync_user];
GO
```
...
Grant read access to the table for the powersync_user:
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/self-hosting/installation/powersync-service-setup#powersync-configuration"
  >
