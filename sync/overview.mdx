...
Each [PowerSync Service](/architecture/powersync-service) instance has a deployed _Sync Rules_ or _Sync Streams_ configuration. This takes the form of a <Tooltip tip="For PowerSync Cloud, the YAML file is edited and deployed to a specific PowerSync instance in the PowerSync Dashboard. For self-hosting setups, the YAML is configured as part of your instance configuration.">YAML file</Tooltip> which contains:
- **In the case of Sync Rules:** Definitions of the different [buckets](/architecture/powersync-service#bucket-system) that exist (basically logical groupings of data), with SQL-like queries to specify the parameters used by each bucket (if any), as well as the data contained in each bucket.
