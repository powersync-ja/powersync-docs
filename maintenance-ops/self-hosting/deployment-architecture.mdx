...
1. 2x PowerSync replication containers, 1GB memory, 1 vCPU each (only one container can replicate at a time; the second provides fast failover).
2. 2+ PowerSync API containers, 1GB memory each, 1 vCPU each. Spread these across multiple hosts or availability zones so a single host failure does not remove all API capacity.
3. A 3-node MongoDB replica set, 2+GB memory each. Refer to the MongoDB documentation for deployment requirements. M10+ when using Atlas. If using Postgres for bucket storage, use a Postgres instance with HA (replication and failover). A single-node bucket storage database is a single point of failure; a failure may require recreating bucket storage and a full re-sync for all clients.
...
Only one replication process is active at any time. If multiple run concurrently, you will see a `[PSYNC_S1003] Sync rules have been locked by another process for replication` error from the standby container. This is expected behavior.

For high availability, run two replication containers. The second container stays on standby and automatically takes over replication if the active container fails. During rolling deploys it is normal to see the lock error briefly while two containers overlap.
...
Set the environment variable `NODE_OPTIONS=--max-old-space-size-percentage=80` to limit the Node.js heap to 80% of the container's available memory. 
...
Set the environment variable `NODE_OPTIONS=--max-old-space-size-percentage=80` to limit the Node.js heap to 80% of the container's available memory. Without a memory limit, the Node.js heap can grow unchecked and lead to OOM kills.
...
The compact job uses up to 1GB memory for compacting, if available. Set the environment variable `NODE_OPTIONS=--max-old-space-size-percentage=80` to limit the Node.js heap to 80% of the container's available memory.
