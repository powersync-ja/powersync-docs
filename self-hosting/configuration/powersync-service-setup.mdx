...
MongoDB requires at least one replica set node. A single node is fine for development/staging environments, but a 3-node replica set is recommended [for production](/self-hosting/advanced/production-setup) deployments.
...
Specify the connection to Postgres in the `replication` section. For details on connecting to your database, see the [Connect PowerSync to your Database](/installation/quickstart-guide#connect-powersync-to-your-database) section in the Quickstart Guide.
...
Your project's [sync rules](/self-hosting/configuration/powersync-service-setup#sync-rules) can either be specified within your configuration file directly, or in a separate file that is referenced.
