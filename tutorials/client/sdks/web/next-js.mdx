...
<Note>PowerSync now supports SSR environments. Importing PowerSync packages no longer throws errors during server-side rendering. The SDK automatically detects SSR environments and returns empty results for queries executed server-side, allowing your pages to render without errors.</Note>
