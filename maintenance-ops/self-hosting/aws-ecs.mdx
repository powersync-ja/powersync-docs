...
- [Bucket Storage](/configuration/powersync-service/self-hosted-instances#bucket-storage-database): Connection details for your bucket storage database (MongoDB or Postgres)
  <Tabs>
...
The task definitions below allocate **2 vCPU and 4GB memory** per container, which is a good starting point. You can adjust resources based on your workload — see [Deployment Architecture](/maintenance-ops/self-hosting/deployment-architecture) for scaling guidance (recommended minimum: 2 vCPU, 2GB memory).
