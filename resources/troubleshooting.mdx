...
<Note>
`OPFSCoopSyncVFS` requires [cross-origin isolation](https://web.dev/articles/cross-origin-isolation-guide) to enable `SharedArrayBuffer`. Add these HTTP headers to your server (local and production):
...
SharedArrayBuffer    // Should return: function SharedArrayBuffer()
```

`Cross-Origin-Opener-Policy: same-origin` may break popup-based OAuth flows. Use redirect-based authentication instead.
</Note>
