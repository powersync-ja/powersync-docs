...
This connection pool uses a design similar to `OPFSWriteAheadVFS`, allowing concurrent reads and a writer to operate on the database in parallel.
...
approach cannot be named. Thus, this connection pool is primarily relevant when all of the following apply:
...
To use it, import `@powersync/web/extra/shared-memory-pool`:
