...
```
Note that the behavior here is different to Sync Rules because `subscription.parameter()` is local to the subscription (so the stream can be subscribed to multiple times with different page numbers, whereas Sync Rules only allow a single global page number at a time). Global parameters (`connection.parameter()`) also still work with streams, but subscription parameters are recommended because they are much more flexible.
