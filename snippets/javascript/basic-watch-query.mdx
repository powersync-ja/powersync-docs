...
<Warning>
  The `db.watch()` AsyncIterator and Callback methods above are only maintained for backwards compatibility. Use the improved `db.query().watch()` API instead (see [Incremental Watch Queries](/client-sdks/watch-queries#incremental-watch-queries) below).
</Warning>
