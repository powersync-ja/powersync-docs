...
PowerSync reads the Postgres WAL using logical replication in order to create [sync buckets](/architecture/powersync-service#bucket-system) in accordance with the specified PowerSync [Sync Rules](/sync/rules/overview).
...
<Tip>For more information on migrating from MongoDB Atlas Device Sync to PowerSync, see our [migration guide](/migration-guides/mongodb-atlas).</Tip>
