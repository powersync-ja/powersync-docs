...
- **Demo Node.js Backend**: The Node backend is the backend for your application and is responsible for generating JWT tokens and API requests that handle upload events from a connected client.
...
## Step 2: Test with the diagnostics app

1. Generate a development token
    - The `Demo Node.js Backend` service has a `/api/auth/token` endpoint you can hit to get a developmentJWT token. You can use this endpoint to generate a development token.
...
2. Open the `PowerSync Diagnostics Client` service in the browser.
