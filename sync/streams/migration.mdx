...
description: How to migrate existing projects from Sync Rules to Sync Streams.
---
...
4. **Simpler syntax**: Just queries with subqueries. No separate parameter queries. The syntax is closer to plain SQL.
...
If you want "sync everything upfront" behavior (like Sync Rules), set `auto_subscribe: true` on your streams and clients will subscribe automatically when they connect.
...
| Swift | [In progress](https://github.com/powersync-ja/powersync-swift/pull/86) | v1.8.0 |
...
| `request.parameters() ->> 'key'` | `connection.parameter('key')` or `subscription.parameter('key')` |
| `bucket.param_name` | Use the parameter directly in the query |
