...
// import { ExpoFileSystemAdapter } from `@powersync/node`;
...
verifyAttachments is called internally during `startSync()`. If you need to manually verify, you can access the internal method or implement your own verification logic by checking attachment records and their local files
...
```swift Swift
// verifyAttachments is called internally during startSync()
// If you need to manually verify, you can wait for initialization:
...
```kotlin Kotlin
// The verify attachment method is currently private in Kotlin SDK

// Example: Manual verification
suspend fun verifyAttachments() {
    attachmentQueue.attachmentsService.withContext { context ->
        val attachments = context.getAttachments()
        attachments.forEach { attachment ->
            if (attachment.localUri != null) {
                val exists = attachmentQueue.localStorage.fileExists(attachment.localUri)
                if (!exists && attachment.state == AttachmentState.SYNCED) {
                    // Handle missing file
                }
            }
        }
...
// TODO add deprecation note about js and dart old packages
