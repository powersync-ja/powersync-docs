...
When multiple streams need the same filtering logic, you can define it once using a Common Table Expression (CTE) and reuse it everywhere. This keeps your configuration DRY and makes it easier to maintain.
...
Define CTEs at the top level to make them available to all streams:
...
2. The same bucket (efficient sync, no duplicate data)
...
**CTE names take precedence over table names.** If you define a CTE with the same name as a database table, the CTE will be used. Choose distinct names to avoid confusion.
