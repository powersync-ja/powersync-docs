...
With [incremental reprocessing](/sync/advanced/incremental-reprocessing) (Beta; currently requires Sync Streams with a MongoDB source database and MongoDB bucket storage), changing one independent stream does not make clients download every other stream again. Clients keep data backed by reused definitions and download data for new or rebuilt definitions.
