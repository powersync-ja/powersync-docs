...
- **PSYNC_S2305**:
  ...
  This code covers two limits per active connection, both with a default of 1,000: the number of unique buckets (`Too many buckets`) and the number of parameter query results returned before deduplication (`Too many parameter query results`). Read the message to see which limit you reached. See [Bucket Count and Limits](/sync/streams/bucket-count) for the full model and reduction strategies, [Too Many Buckets (Troubleshooting)](/debugging/troubleshooting#too-many-buckets-psync_s2305) for quick diagnosis, and [Performance and Limits](/resources/performance-and-limits) for the limit details.
