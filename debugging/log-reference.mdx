...
On `Sync stream complete`, `operations_synced` and `data_synced_bytes` are session totals. On checkpoint entries, these fields count only the operations and bytes since the previous reported counts, or since the session started. The Service counts bytes before compression.
