...
- **PSYNC_S2305**:
  ...
  This code covers two limits per active connection, both with a default of 1,000: the number of unique buckets (`Too many buckets`) and the number of parameter query results returned before deduplication (`Too many parameter query results`). Read the message to see which limit you reached, then see [Reducing Bucket Count](/sync/streams/reducing-bucket-count) to diagnose and resolve this error.
