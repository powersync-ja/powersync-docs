...
**Using Supabase?** If you are using [Supabase](https://supabase.com/) as your backend, we provide a [PowerSync\<>Supabase](/integrations/supabase/guide) integration guide which includes a tutorial and demo app to quickly learn how to use PowerSync with Supabase.
...
<Note>
  For more details on database setup, including provider-specific guides (Supabase, AWS RDS, etc.), see [Source Database Setup](/configuration/source-db/setup).
</Note>
...
For more advanced use cases like user-specific data, see the [Sync Rules documentation](/sync/rules/overview).
...
- Explore [Live Queries](/client-sdks/watch-queries) for reactive UI updates
- Check out [Example Projects](/intro/examples) for complete implementations
- Review the [Client SDK References](/client-sdks/overview) for platform-specific details
