...
### Postgres schema changes affecting Sync Streams / Sync Rules

#### DROP table

Dropping a table is not directly detected by PowerSync, and previous data may be preserved. To make sure the data is removed, `TRUNCATE` the table before dropping, or remove the table from your [Sync Streams](/sync/streams/overview) (or [Sync Rules](/sync/rules/overview)).
...
Renaming a synced collection to a name that _is not included_ in Sync Streams or Sync Rules has the same effect as dropping the collection.
...
handled by redeploying your [Sync Streams](/sync/streams/overview) (or [Sync Rules](/sync/rules/overview)). This triggers a re-replication.

### MySQL schema changes affecting Sync Streams / Sync Rules
