...
When several tables share the same access pattern, you can group them into a single stream using multiple queries. A single stream syncs data more efficiently and the client only needs to manage one subscription instead of multiple.
