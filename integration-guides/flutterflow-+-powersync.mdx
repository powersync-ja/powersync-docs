...
This guide takes you through building a basic app from scratch and takes about 30-40 minutes to complete. The app lets you manage a list of items. You should then be able to use this knowledge to build and extend your own app.
...
4. Reading data
5. Creating data
6. Updating data (Coming soon)
7. Deleting data
8. Signing out
9. Displaying Connectivity and Sync Status
10. Securing your app
...
3. Create a Postgres publication using the SQL Editor. This will enable data to be replicated from Supabase so that your FlutterFlow app can download it.
...
1. To update your Sync Rules, open the `sync-rules.yaml` file.
   <Frame>
   <img src="/images/integration-22.avif" width="60%"/>
   </Frame>
...
7. Still under the **Widget Tree**, select the “ListView” widget.
    1. Click on “Generate Dynamic Children”
...
2. Instant Reload your app or start another test session.
...
1. Under **"Custom Code"**, create a new Custom Action called `signOut` without Arguments or Return Values and paste the below code:
...
## (Optional) Displaying Connectivity and Sync Status
