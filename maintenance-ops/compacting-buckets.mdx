...
With [incremental reprocessing](/sync/advanced/incremental-reprocessing) (Beta; currently requires Sync Streams with a MongoDB source database and MongoDB bucket storage), PowerSync reuses unchanged compiled definitions. This reduces reprocessing during the deployment and prevents clients from downloading unchanged data again. Reused buckets also keep their operation history, so a deployment no longer acts like defragmenting every bucket.
