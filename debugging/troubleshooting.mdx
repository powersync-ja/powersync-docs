...
* **Dart/Flutter SDK** - Logging is enabled by default and outputs logs to the console in debug mode.
...
Sync & API logs in the [PowerSync Dashboard](https://dashboard.powersync.com/) record a **Sync stream started** event when a client connects and a **Sync stream complete** event when the session ends. The complete event includes how many operations were synced, how much data was transferred, and how long the connection stayed open. See [Session Fields](/debugging/log-reference#session-fields) for field meanings.
