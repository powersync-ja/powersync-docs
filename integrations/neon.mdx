...
description: "Build apps on Neon with PowerSync that use client-side SQLite for real-time updates across users and devices, instant interactions and continued functionality when offline."
sidebarTitle: Neon
---
...
This guide provides instructions for how to configure PowerSync for use with your [Neon](https://neon.tech/) project using a TypeScript demo app.
...
[Sync Streams](/sync/streams/overview) allow developers to control which data gets synced to which user devices using a SQL-like syntax in a YAML file. For the demo app, we're going to specify that each user can only see their own notes (plus any shared notes).

1. In the PowerSync Dashboard, select your project and instance and go to the **Sync Streams** view.
...
During development, you can use the **Sync Test** feature in the PowerSync Dashboard to validate your Sync Streams:
...
3. Click **"Launch Sync Diagnostics Client"** to test the Sync Streams.
