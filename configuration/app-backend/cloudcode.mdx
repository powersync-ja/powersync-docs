...
We've made optional functionality available to MongoDB customers that handles writing to a backend source database and generating JWTs.

This makes PowerSync easier to implement for developers migrating from MongoDB Atlas Device Sync who prefer not having to maintain their own backend code and infrastructure (PowerSync's [usual architecture](/configuration/app-backend/setup) is to use your own backend to process writes and generate JWTs).
