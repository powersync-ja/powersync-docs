...
The `org_data` stream combines three queries that all filter by the user's organizations. They sync together as one unit. The `project_details` stream uses a [stream-scoped CTE](/sync/streams/ctes#stream-scoped-ctes) and groups tasks, files, and comments for a specific project into a single subscription.
