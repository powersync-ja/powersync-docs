...
With a standard setup, PowerSync Cloud provides “bucket storage” (persistent database storage where sync bucket data such as operation history and metadata are stored by the PowerSync Service) as part of the cloud service. For HIPAA compliant setups, however, the customer must supply a dedicated MongoDB Atlas cluster to serve as the bucket storage database for the PowerSync Service instance(s).
...
* **PowerSync Dashboard Account**  
  Customers are in full control of their PowerSync Cloud account and are responsible for managing the users who have access to the PowerSync Dashboard. MFA must be enabled for the PowerSync Dashboard. 
...
A strong **SOC 2 Type 2 report** provides independent assurance that PowerSync maintains the necessary security posture to meet the administrative and technical safeguards required for a HIPAA Business Associate.
