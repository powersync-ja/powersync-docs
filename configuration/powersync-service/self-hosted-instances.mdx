...
Your Sync Streams (or legacy Sync Rules) configuration can either be specified within the config file directly, or in a separate file that is referenced. The `sync_config:` key is used for both Sync Streams and Sync Rules:
