...
<Tabs>
  <Tab title="Sync Streams">
    ...
    <Info>
      With Sync Streams, parameters are accessed directly in queries using `auth.user_id()`, `auth.parameter()`, `subscription.parameter()`, and `connection.parameter()`. See the [Sync Streams documentation](/usage/sync-streams.md) for details.
    </Info>
  </Tab>
