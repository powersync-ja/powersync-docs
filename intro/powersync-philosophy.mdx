...
description: "Our vision is that a local-first or offline-first app architecture should be easier for the developer to implement than cloud-first, and give a better experience for the end-user — even when they're online."
---
...
You use a standard Postgres, MongoDB, MySQL, or SQL Server \[[1](#footnotes)\] database on the server, a standard SQLite database on the client, and your [own backend](/configuration/app-backend/setup) to process writes. PowerSync simply keeps the SQLite database in sync with your backend database.
