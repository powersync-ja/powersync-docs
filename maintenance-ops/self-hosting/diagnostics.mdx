...
If you have the [PowerSync CLI](/tools/cli) installed, use `powersync fetch status` to check instance status without calling the API directly. This works with any running PowerSync instance — local or remote.

```bash
powersync fetch status

# Extract a specific field
powersync fetch status --output=json | jq '.connections[0]'
```
