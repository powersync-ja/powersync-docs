...
description: Learn how Sync Streams and Sync Rules enable partial sync to control which data gets synchronized to each client.
---

PowerSync Sync Streams and the legacy Sync Rules allow developers to control which data gets synchronized to which clients/devices (i.e. they enable partial sync).
...
<Tip>
If you're currently using Sync Rules and want to migrate to Sync Streams, see our migration guide (coming soon).
</Tip>
...
Each [PowerSync Service](/architecture/powersync-service) instance has a deployed _Sync Streams_ or _Sync Rules_ configuration. This takes the form of a <Tooltip tip="For PowerSync Cloud, the YAML file is edited and deployed to a specific PowerSync instance in the PowerSync Dashboard. For self-hosting setups, the YAML is configured as part of your instance configuration.">YAML file</Tooltip> which contains:
...
<Note>
A _parameter_ is a value that can be used in Sync Streams or Sync Rules to create dynamic sync behavior for each user/client. Each client syncs only the relevant [_buckets_](/architecture/powersync-service#bucket-system) based on the parameters for that client.
...
It is also possible to have buckets/streams with no parameters. These sync to all users/clients.
</Note>
...
* In _Sync Streams_, buckets and parameters are implicit — they are automatically created based on the streams, their queries and subqueries. You don't need to manage buckets directly.
