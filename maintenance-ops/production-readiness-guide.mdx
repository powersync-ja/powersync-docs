...
description: "Best practices for running PowerSync reliably in production."
...
3. `uploading` - Boolean; If the client has a network connection and changes in the upload queue are present this will be set to true when the client attempts to upload changes to the backend API in the `uploadData` function.
4. `downloading` - Boolean; If the client is connected to the PowerSync Service and new data is available, this will be set to true, else it will be false.
...
- [JavaScript](https://powersync-ja.github.io/powersync-js/common/interfaces/SyncStatus) (re-exported from all SDK packages)
...
If you're interested in learning more about the internal PowerSync SQLite schema, see the [Client Architecture](/architecture/client-architecture#client-side-schema-and-sqlite-database-structure) section of the docs.
  ...
  statusChanged: (status) => {
    ...
    if(status.downloadError) {
      logger.error("PowerSync sync download failed", {
        ...
        downloadError: status.downloadError // The actual download error details
      });
    }
    ...
    if(status.uploadError) {
      logger.error("PowerSync sync upload failed", {
        ...
        connected: status?.connected,            // Network connection status
        sdkVersion: powerSync.sdkVersion || 'unknown', // PowerSync SDK version for debugging
        uploadError: status.uploadError   // The actual upload error details
      });
    }
  }
});
...
Once configured, create a [Webhook](/maintenance-ops/monitoring-and-alerting#webhooks) alert or [Email](/maintenance-ops/monitoring-and-alerting#email-rules) notifications to ensure you are notified when replication issues arise.
...
#### Managing and Monitoring Replication Lag

<Note>
  For a broader overview of replication lag across source databases, how to monitor it, and common causes, see [Replication Lag](/maintenance-ops/replication-lag).
</Note>
...
2. Sync Streams/Sync Rules complexity - This also impacts the time it takes to complete the initial replication.
...
During a snapshot, PowerSync warns when less than 50% of the WAL budget remains. You may see this warning in the PowerSync dashboard, in the [Diagnostics API](/maintenance-ops/self-hosting/diagnostics) if you self-host, and in PowerSync Service logs. The [Log Reference](/debugging/log-reference#postgres-replication-slots-and-wal-budget) explains the slot and WAL budget messages. Increase `max_slot_wal_keep_size` or reduce snapshot work before the slot is invalidated. Use the considerations above to set a high enough cap.
