...
<Tab title="Kotlin Compose">
  The `composeSyncStream` extension subscribes to a stream when the composable enters the composition and automatically unsubscribes when it is disposed. It returns `SyncStreamStatus?` for the subscription so you can check sync state.
