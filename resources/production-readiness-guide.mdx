...
Implementing logging in your client application is essential for diagnosing issues that end users may encounter. Collecting details such as `user_id`, `timestamp`, `log_level`, `message`, `sdk_version`, and `raw_log_message` can provide valuable context when troubleshooting problems. There are several approaches to enabling client-side logging.

### Option 1: Using an `insertOnly` Table

<Warning>Important Warning: When using insertOnly tables for logging, do not set the log level to `DEBUG` in production. Debug-level logging generates an enormous volume of logs that would be extremely resource-intensive to upload and could overwhelm your backend API and/or client. Stick to `WARN` or `ERROR` levels for production logging.</Warning>
...
logger.useDefaults();
// Use WARN or higher in production to avoid excessive log volume
...
logger.setHandler((messages, context) => {
  ...
  powerSync.execute(
    `INSERT INTO log (id, message, level, timestamp, raw) VALUES (uuid(), ?, ?, date(), ?)`, 
    [
      message,
      level,
...
// Usage examples
logger.warn('PowerSync connection failed', { userId: user.id, error: error.message });
...
```
### Option 2: Using something like `Sentry` for Log Aggregation (Recommended)
...
logger.setHandler((messages: any, context) => {
  ...
  // Normalize messages into an array first
  const msgs: any[] = Array.isArray(messages) ? messages : [messages];
  ...
  const level = context.level?.name ?? "UNKNOWN";
...
logger.error('PowerSync sync failed', {
              ...
              sdk_version: powerSync.sdkVersion || 'unknown',
            });
```
