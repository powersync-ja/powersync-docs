...
- `auth.user_id()` in [Sync Streams](/usage/sync-streams)
- `request.user_id()` (previously `token_parameters.user_id`) in [Sync Rules (legacy)](/usage/sync-rules)

If you compare these to your Sync Streams configuration in `sync-rules.yaml`, you'll see they are quite similar.
