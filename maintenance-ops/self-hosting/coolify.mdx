...
description: "Deploy the self-hosted PowerSync Service on Coolify with Docker Compose."
---
...
<AccordionGroup>
    <Accordion title="Step 1: Create Docker Compose Empty resource">
        Add the [`Compose file`](#base-compose-file) as a Docker Compose Empty resource to your project. 
        <video
            controls
            ...
            <Tab title="Environment variables">
                 <table>
                    ...
                    <tbody>
                        <tr>
                            <td><code>PS_DATABASE_TYPE</code></td>
                            <td><code>postgresql</code> OR <code>mongodb</code> OR <code>mysql</code> OR <code>mssql</code></td>
                        </tr>
                        <tr>
                            <td><code>PS_DATABASE_URI</code></td>
...
<AccordionGroup>
  ...
  <Accordion title="Step 2: Update environment variables" defaultOpen={false}>
    ...
    [Environment Variables](/configuration/powersync-service/self-hosted-instances#environment-variables).
    <Frame>
  ...
  <Accordion title="Step 3: Update config files" defaultOpen={false}>
        Navigate to the `Storages` tab and update the `sync-config.yaml` and `service.yaml` files as needed.
        ...
        <Steps>
            <Step title="Update the `sync-config.yaml` file">
                <Frame>
                    <img src="/images/coolify/powersync_sync_rules.png"/>
                </Frame>
            </Step>

            <Step title="Update the `service.yaml` file">
                <Frame>
