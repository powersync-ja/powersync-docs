...
The **Search** input above the log list accepts free-text terms and structured `alias:value` filters. Type a query and the results update as you type. Applied filters appear as removable chips below the input when you collapse it.

#### Free-Text Search

Type bare words to match anywhere in the raw log entry. Matching is case-insensitive, and each word matches independently:
...
Use `alias:value` to filter on a specific structured field. Unlike free-text search, a field filter only matches the named field, not the rest of the log entry. Repeating an alias matches any of the listed values (OR within a field, AND across fields):

```
user:<user-id-a> user:<user-id-b> error:PSYNC_S2001
```

Quote values that contain spaces:

```
reason:"client closing stream"
```

The following aliases are supported for API logs:

| Alias     | Field          | Description                                              |
| --------- | -------------- | -------------------------------------------------------- |
| `user`    | `user_id`      | The authenticated user ID for the sync session.          |
| `client`  | `client_id`    | The client device ID.                                    |
| `request` | `rid`          | The request ID. Matches a started or complete entry.     |
| `error`   | `error_code`   | A specific PSYNC error code (see [Error Codes](/debugging/error-codes)). |
| `reason`  | `close_reason` | Why the sync session closed.                             |
...
Filters on fields the dashboard doesn't have a built-in alias for pass through to the service unchanged. For example:

```
buckets:1
```

The chip for an unknown filter renders with a different color so you can spot it. The service evaluates the filter against the matching field on each log entry.
...
```
checkpoint sync user:123 checkpoint:>=1224
```

This matches entries containing `checkpoint` or `sync` in the message, with `user_id` equal to `123` and a `checkpoint` field greater than or equal to `1224`.

#### Editing Filters

Each active filter renders as a chip below the search input when the input is collapsed:

* Click the chip body to edit the value.
