...
<Note>
  ...
  From v1.14 onward, the Kotlin dependency has been removed entirely. The SDK is now implemented natively in Swift, with the PowerSync sync protocol and SQLite extension handled by a small Rust core. 
</Note>
...
<Warning>
  **Upgrading to v1.14**: This release removes the internal Kotlin dependency and reimplements the SQLite connection pool natively in Swift. The public API is unchanged, but the pool internals are a full rewrite — we recommend testing queries and CRUD upload flows in your app after upgrading.
</Warning>
