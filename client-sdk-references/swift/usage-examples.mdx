...
struct SyncProgressBar: View {
    ...
    var priorityProgress: Float {
        db.currentStatus.downloadProgress?.untilPriority(priority: BucketPriority(2)).fraction ?? 1.0
    }
    ...
    var body: some View {
        if progress < 1.0 {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Priority 2 progress: \(Int(priorityProgress * 100))%")
                ProgressView(value: priorityProgress)
                    .progressViewStyle(LinearProgressViewStyle())
            }
            .padding(.vertical)
        }
