...
Convex does not support user-defined database schemas or namespaces in the same way as SQL databases. In Sync Streams, use the default `convex` schema when qualifying source tables.
...
To keep synced values consistent, cast Convex `Int64` fields to `TEXT` in your Sync Streams. See the examples below.
