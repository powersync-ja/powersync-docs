...
<Info>
This feature is currently in a beta release and available for our [Flutter](/client-sdk-references/flutter) and JavaScript SDKs ([React Native](/client-sdk-references/react-native-and-expo) and [Web](/client-sdk-references/js-web)).

Support for passing client parameters from our Kotlin Multiplatform and Swift SDKs will follow soon.

</Info>

<Warning>
  Use client parameters with caution. Please make sure to read the [Security consideration](/usage/sync-rules/advanced-topics/client-parameters-beta#security-consideration) section below.
</Warning>
...
Client parameters are defined when [instantiating the PowerSync database](/usage/installation/client-side-setup/instantiate-powersync-database), within the options of PowerSync's `connect()` method:
