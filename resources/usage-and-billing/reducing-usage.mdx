...
With [incremental reprocessing](/sync/advanced/incremental-reprocessing) (currently in Beta; supported only with Sync Streams, a MongoDB source database, and MongoDB bucket storage), changing one independent stream does not make clients download every other stream again. Clients keep their data for unchanged streams and download only the data for new or changed streams.

Without incremental reprocessing, PowerSync prepares a complete new set of sync data after each deployment. Clients then download all data selected by the new config again. In both cases, bucket storage can grow temporarily while PowerSync prepares the change. Group related changes when practical instead of deploying many small updates.
