...
Encryption support is available for PowerSync's Swift SDK using [`SQLite3MultipleCiphers`](https://utelle.github.io/SQLite3MultipleCiphers/) via the `initialStatements` parameter on `PowerSyncDatabase()`. This allows you to run `PRAGMA key` statements to configure encryption when databases are opened.
