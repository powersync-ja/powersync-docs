...
Full support for PowerSync Cloud. Log in with a personal access token, then define your instance config in a `powersync/` directory (`service.yaml`, `sync-config.yaml`) in your project and deploy with `powersync deploy`.
...
- **Linking and scope:** Link to an existing PowerSync Service instance and run a subset of commands (e.g. `powersync status`, `powersync generate schema`, `powersync validate`). The CLI does not provision or deploy to a remote server; for local development, use **Docker** to bootstrap a project and run a local PowerSync Service (and optional DB/storage).
- **Setup:** Run `powersync init self-hosted` to scaffold a config directory. Edit `service.yaml` with your instance details (including `api.tokens` for API key auth), then run `powersync link self-hosted --api-url <url>` to link to your instance. The link writes `cli.yaml`; set `api_key` there (e.g. `api_key: !env PS_ADMIN_TOKEN`) or set the `PS_ADMIN_TOKEN` environment variable. When using Docker (see below), `powersync docker configure` links to the local API automatically.
- **Supported commands:** `powersync status`, `powersync generate schema`, `powersync generate token`, `powersync validate`, `powersync fetch instances` (lists linked instances by scanning the current directory for folders with `cli.yaml`).
