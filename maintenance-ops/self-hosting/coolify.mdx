...
description: "Guide for deploying the [PowerSync Service](http://localhost:3333/architecture/powersync-service) on Coolify"
---
...
- `sync_config.yaml` file (according to your data requirements)
- `powersync.yaml` file
...
<Accordion title="Compose file">
    ...
    powersync:
        ...
        - type: bind
            source: ./volumes/config/sync_config.yaml
            target: /home/config/sync_config.yaml
            content: |
            config:
                edition: 3
            streams:
        ...
        - type: bind
            ...
            # Specify Sync Streams (or Sync Rules)
            sync_config:
                path: /home/config/sync_config.yaml
...
<AccordionGroup>
  ...
  <Accordion title="Step 3: Update config files" defaultOpen={false}>
        Navigate to the `Storages` tab and update the `sync_config.yaml` and `powersync.yaml` files as needed.
        ...
        <Steps>
            <Step title="Update the `sync_config.yaml` file">
                <Frame>
