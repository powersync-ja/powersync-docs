...
| **Authentication integration:** (optional) | PowerSync uses JWTs for authentication between the Client SDK and PowerSync Service. Some [authentication providers](/configuration/auth/overview) already generate JWTs for users which PowerSync can verify directly. For others, some backend code must be added to your application backend to generate the JWTs. |
...
| **Authentication integration:** | `fetchCredentials()` | This is called every couple of minutes and is used to obtain a JWT. The PowerSync Client SDK uses that JWT to authenticate against the PowerSync Service. See [Authentication Setup](/configuration/auth/overview) for more details. |
