{/* Shared body: rendered by sync/advanced/storage-version-4.mdx (Sync Streams section) and sync/rules/storage-version-4.mdx (Sync Rules (Legacy) section). Keep the content valid for both engines. */}
