...
description: "Learn how to implement custom conflict resolution strategies in PowerSync to handle concurrent updates from multiple clients."
---

The default behavior is essentially "last write wins" because the server processes operations in order received, with later updates overwriting earlier ones. For many apps, this works fine. But some scenarios demand smarter handling.
...
interface CrudEntry {
  ...
  previousValues?: Record<string, any>;  // Previous values (trackPreviousValues)
}
```
...
bucket_definitions:
  user_data:
    parameters:
      - user_id
    data:
      - SELECT * FROM tasks WHERE user_id = request.user_id()
      - SELECT * FROM write_conflicts 
