...
<Tab title="React Hooks">
  The `useSyncStream` hook automatically subscribes when the component mounts and unsubscribes when it unmounts. Use this when you know which single stream a component needs:
  ```jsx
  ...
  ```
   ...
   When the number of streams is only known at runtime - for example, it depends on an array from props or state - use `useSyncStreams` (plural) instead. It accepts an array of stream options and returns a matching array of statuses. When the array contents change or the component unmounts, all previous subscriptions are unsubscribed before new ones are created.

  ```jsx
  function ProjectView({ listIds }) {
    // Subscribe to a dynamic number of streams
