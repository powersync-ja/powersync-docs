...
A role is a named set of permissions that controls what a member can see and do in your organization. You assign a role when inviting a member, and you can change a member's roles at any time. A member can hold more than one role. Manage roles from the **Roles** tab on the **Team** page.

<Frame caption="The Roles tab on the Team page, listing built-in and custom roles with their grants and assigned members">
  ![](/images/usage/tools/dashboard-team-roles.png)
</Frame>

### Built-in Roles

Every organization includes built-in roles that you can assign but cannot edit or delete:

- **Owner** - Full access to the organization, its projects, instances, team, and billing.
- **Developer** - A broad working set of permissions for building and operating projects and instances.

You can see the exact grants for any role in the Roles tab. To grant access that differs from the built-in roles, create a custom role.

### Custom Roles

Custom roles are available on the [Team and Enterprise plans](https://www.powersync.com/pricing).

A custom role combines a set of permissions with a project scope. Each permission covers one area of the dashboard and has one or more access levels. A higher level includes everything the lower level allows. A role needs at least one permission.
...
| Notification configurations | View, Manage | View notification configurations. Manage adds creating, editing, deleting, and testing them. |
...
A custom role applies to either all projects in the organization or to a specific set of projects that you choose. Scope affects the permissions that operate at the project level: Projects, Instances, Instance logs, Alert rules, and Notification configurations. The remaining permissions (Private Endpoints, Team, Billing, and Organization settings) always apply to the whole organization.

A project-scoped member is granted their permissions inside the selected projects and has no access outside them. The Manage level on Projects is only available on roles scoped to all projects, since creating and deleting projects is an organization-wide action.

### How Permissions Appear in the Dashboard

Members only see and do what their roles allow. Where a member lacks permission for an action, the related control stays visible but disabled, so it is clear which capabilities an administrator could grant. Opening a page the member cannot view shows an access message rather than the page content. The dashboard reflects permissions as a convenience. Access is always enforced by the PowerSync backend.
