...
CDC must be enabled for the specific tables that need to be replicated:
...
Create a database user that has:
- Read access to the replicated tables (`db_datareader` role)
- Read access to the CDC tables (`cdc_reader` role)

```sql
-- Create a user for PowerSync
CREATE USER powersync_user WITH PASSWORD = 'your_secure_password';

-- Grant read access to replicated tables
...
Alternatively, you can grant more granular permissions:

```sql
-- Grant SELECT on specific tables
GRANT SELECT ON dbo.your_table_name TO powersync_user;

-- Grant access to CDC schema
GRANT SELECT ON SCHEMA::cdc TO powersync_user;
```

### CDC Management

Management of CDC is up to the developer, though there are some settings of interest to PowerSync:
