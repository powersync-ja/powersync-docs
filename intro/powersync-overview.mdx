...
description: "PowerSync keeps your backend database in sync with on-device SQLite for instant, offline-capable apps."
sidebarTitle: Introduction
---

import ClientSdks from '/snippets/client-sdks.mdx';

PowerSync syncs your backend database to a local SQLite database on the device. Your app reads and writes directly to local SQLite with no API calls needed for data access, so apps respond instantly and remain fully functional without a network connection. 

The PowerSync Service replicates from your backend database (Postgres, MongoDB, MySQL, or SQL Server) and streams updates to clients through the PowerSync SDK.
