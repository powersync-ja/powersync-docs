...
<AccordionGroup>
  <Accordion title="Where can I review my PowerSync Service usage?">
    ...
    - Organization-wide usage: Navigate to your organization in the [PowerSync Dashboard](https://dashboard.powersync.com/) and check the **Plan Usage** section for aggregated metrics across all instances in your current billing cycle.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Where can I see details about my current billing cycle?">
    Navigate to your organization in the [PowerSync Dashboard](https://dashboard.powersync.com/) and go to the **Plans Usage** section. Here you can view your total usage (aggregated across all projects in your organization) for your current billing cycle. Data in this view updates once a day.
  </Accordion>
  <Accordion title="Where can I update my billing details, e.g. the email that receives billing receipts?">
    You can update your billing details in the **Plans & Billing** section of the [PowerSync Dashboard](https://dashboard.powersync.com/) at the organization level.
  </Accordion>
  <Accordion title="Can I view my historic invoices?">
    You can review your historic invoices directly in the Stripe Customer Portal, by signing in with your billing email [here](https://billing.stripe.com/p/login/7sI6pU48L42cguc7ss). We may surface these in the Dashboard in the future.
  </Accordion>
...
1. **New App Installations:**
   ...
   - Providing manual [defragmentation options](/usage/lifecycle-maintenance/compacting-buckets#defragmenting) (in the PowerSync Dashboard)
2. **Existing Users:**
...
3. **Sync Rule Deployments:**
   ...
   We are working on [incremental sync rule reprocessing](https://roadmap.powersync.com/c/85-more-efficient-sync-reprocessing), which will allow PowerSync to only reprocess buckets whose definitions have changed, rather than all buckets.
4. **Unsynced Columns:**
...
Data synced measures the total uncompressed bytes streamed from the PowerSync Service to clients. Spikes typically come from either lots of sync operations (high churn) or large rows (wide payloads), and can also occur during first-time syncs, defragmentation, or Sync Rule updates.
...
2. Inspect instance logs for size
   ...
   - You may need to scroll to load more logs.
   - [Contact us](/resources/contact-us) if you require a CSV export of your logs for a limited time-range. For certain scenarios these could be easier to search than the instance logs in the dashboard.
3. Compare operations vs row sizes
   ...
   - If operations are moderate but size is large, data in your rows is likely large (e.g. large JSON columns or blobs).
...
5. Consider recent maintenance and app changes
   ...
   - New app installs trigger initial full sync; expect higher usage when onboarding new sets of users.
...
## 2. **Review Logs**
   ...
   - Search for "Sync stream complete" entries (use your browser's search function) to review:
     ...
     - You may need to scroll to load more logs.
     - [Contact us](/resources/contact-us) if you require a CSV export of your logs for a limited time-range. For certain scenarios these could be easier to search than 
## 3. **Compare Metrics**
