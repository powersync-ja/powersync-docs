...
With [incremental reprocessing](/sync/advanced/incremental-reprocessing), changing one stream rebuilds only the sync data selected by that stream. This reduces reprocessing during the deployment and prevents clients from downloading unchanged streams again. Buckets for unchanged streams also keep their operation history, so a deployment no longer acts like defragmenting every bucket.
