...
Use this method to wait for a specific priority level to complete syncing. In this example we are rendering a lists component only once the user's lists (with priority 1) have completed syncing:
...
## `SyncStatus.statusInPriority(priority: int)`

This property provides details on the last sync time per priority level.
