...
description: "[Buckets](/architecture/powersync-service#bucket-system) store data as a history of changes, not only the current state."
---
...
## Sync Streams / Sync Rules Deployments

Whenever modifications to [Sync Streams](/sync/streams/overview) or [Sync Rules](/sync/rules/overview) are deployed, all buckets are re-created from scratch. This has a similar effect to fully defragmenting and compacting all buckets. This was recommended as a workaround before explicit compacting became available ([released July 26, 2024](https://releases.powersync.com/announcements/bucket-compacting)).
