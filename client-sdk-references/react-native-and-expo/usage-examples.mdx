...
export function SyncProgressBar({ powersync }) {
  ...
  return (
    <View style={{ marginVertical: 16 }}>
      <Text>Syncing data... {Math.round(progress * 100)}%</Text>
      <ProgressBar progress={progress} style={{ height: 8, borderRadius: 4 }} />
      {/* Example for a specific priority */}
      <Text>Priority 2 progress: {Math.round(priorityProgress * 100)}%</Text>
      <ProgressBar progress={priorityProgress} style={{ height: 8, borderRadius: 4 }} />
    </View>
  );
}
```
