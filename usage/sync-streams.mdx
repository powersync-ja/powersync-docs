...
**Subscription Parameters**: Passed from the client when it subscribes to a Sync Stream. See [Client-Side Syntax](#client-side-syntax) below. Clients can subscribe to the same stream multiple times with different parameters:
...
<Tabs>
  <Tab title="JS">
    ...
    Additionally, the `useQuery` hook for React can wait for Sync Streams to be complete before running queries. Pass `streams` only when the component knows which specific stream subscription(s) it depends on and it should wait before querying.
  ...
  <Tab title="Kotlin">
    ...
    If you're using Compose, you can use the `composeSyncStream` extension to subscribe to a stream while a composition is active:
...
<Tabs>
  <Tab title="JS">
      Try the [`react-supabase-todolist-sync-streams`] demo app by following the instructions in the README.
