...
description: "Map local UUIDs to server-assigned sequential IDs when your backend requires integer primary keys."
...
<Steps>
    ...
    <Step title={"Update Sync Streams"}>
        Update your Sync Streams to use the UUID column instead of the integer ID.
    </Step>
...
With the schema updated, we now need a method to sync and map the `list_id` and `list_uuid` in the `todos` table, with the `id` and `uuid` columns in the `lists` table.
...
can move on to updating your Sync Streams to use the UUID column instead of the integer ID.
