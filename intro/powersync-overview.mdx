...
Copy this prompt into an AI coding agent (Claude Code, Cursor, Codex, or similar) to add PowerSync to an existing project. The agent inspects your project, confirms the right SDK with you, and gathers the details it needs before writing code. Each [client SDK reference](/client-sdks/overview) also has a version of this prompt tailored to that SDK.
