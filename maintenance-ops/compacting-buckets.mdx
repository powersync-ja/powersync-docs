...
On [storage version 4](/sync/advanced/storage-version-4), the experimental `compact --incremental-only` option runs only the lower-cost incremental compacting steps, so you can schedule it more often. Keep the regular full `compact` job as well.
