...
<Warning>
  ...
  The SDK is largely built on our stable [Web SDK](/client-sdks/guides/javascript-web), so that functionality can be considered stable. However, the [Capacitor Community SQLite](https://github.com/capacitor-community/sqlite) integration for mobile platforms is in alpha for real-world testing and feedback. There are [known limitations](#limitations) currently.
</Warning>

<Note>
  **Built on the Web SDK**

  The PowerSync Capacitor SDK is built on top of the [PowerSync Web SDK](/client-sdks/guides/javascript-web). It shares the same API and usage patterns as the Web SDK. The main differences are:
  ...
  All code examples from the Web SDK apply to Capacitor — use `@powersync/web` for imports instead of `@powersync/capacitor`. See the [JavaScript Web SDK reference](/client-sdks/guides/javascript-web) for ORM support, SPA framework integration, and developer notes.
</Note>
...
See the [JavaScript Web SDK reference](/client-sdks/guides/javascript-web) for:

- [ORM Support](/client-sdks/orms/js/overview)
- [SPA Framework Integration](/client-sdks/guides/javascript-web#single-page-application-spa-frameworks)
