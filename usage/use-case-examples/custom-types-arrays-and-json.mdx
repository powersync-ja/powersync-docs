...
<Tabs>
  ...
  <Tab title=".NET">
    ...
    new Table(
      ...
      columns: new[]
      {
        new Column("custom_payload", ColumnType.Text),
        // ... other columns ...
      },
...
<Tabs>
  ...
  <Tab title=".NET">
    ...
    {
      var oldJson = JsonSerializer.Deserialize<Dictionary<string, object>>(
        op.PreviousValues.GetValueOrDefault("custom_payload", "{}")?.ToString() ?? "{}"
      );
      var newJson = JsonSerializer.Deserialize<Dictionary<string, object>>(
        op.OpData.GetValueOrDefault("custom_payload", "{}")?.ToString() ?? "{}"
      );
      var metadata = op.Metadata; // Access metadata here
      // Compare oldJson and newJson to determine what changed
      // Use metadata as needed as you process the upload
...
<Tabs>
  ...
  <Tab title=".NET">
    ...
    new Table(
      ...
      columns: new[]
      {
        new Column("unique_identifiers", ColumnType.Text),
        // ... other columns ...
      },
      options: new TableOptions
      {
...
<Tabs>
  ...
  <Tab title=".NET">
    ...
    if (op.Op.ToString() == "PUT" && op.PreviousValues != null)
    {
      var oldArray = JsonSerializer.Deserialize<List<string>>(
        op.PreviousValues.GetValueOrDefault("unique_identifiers", "[]")?.ToString() ?? "[]"
      );
      var newArray = JsonSerializer.Deserialize<List<string>>(
        op.OpData.GetValueOrDefault("unique_identifiers", "[]")?.ToString() ?? "[]"
      );
      var metadata = op.Metadata; // Access metadata here
      // Compare oldArray and newArray to determine what changed
      // Use metadata as needed as you process the upload
...
Depending on whether the `custom_postgres_types` [compatibility option](/usage/sync-rules/compatibility) is enabled,
...
<Tabs>
  ...
  <Tab title=".NET">
    ...
    new Table(
      ...
      columns: new[]
      {
        new Column("location", ColumnType.Text),
        // ... other columns ...
      },
      options: new TableOptions
      {
