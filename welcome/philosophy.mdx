...
You use a standard Postgres \[[1](/overview/powersync-philosophy#footnotes)\] database on the server, a standard SQLite database on the client, and your own backend to process writes. PowerSync simply keeps the SQLite database in sync with your Postgres database.
...
You use plain Postgres on the server — no extensions, and no significant change in your schema required \[[2](/overview/powersync-philosophy#footnotes)\]. We [use the Postgres WAL](/usage/installation/database-setup) to replicate changes to the [PowerSync Service](/architecture/powersync-service), where data is transformed and partitioned according to sync rules, and persisted in a way that allows streaming incremental changes to each client.
