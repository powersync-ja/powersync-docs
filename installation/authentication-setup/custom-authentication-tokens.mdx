...
Using your newly created shared secret, you can generate JWT token's using the same KID you set in the PowerSync Service configuration. Here's an function using the [`jose`](https://github.com/panva/jose) library:
...
export const generateToken = async (payload: Record<string, unknown>, userId: string) => {
  return await new jose.SignJWT(payload)
    .setProtectedHeader({ alg: 'HS256', kid: 'powersync-load-testing' })
    .setSubject(userId)
...
This token can then be used to authenticate with the PowerSync Service. Client application will need to return the token in the `fetchCredentials()` function and should retieve the token from your custom authentication service.