...
The default Write Checkpoints implementation relies on uploads being acknowledged _synchronously_, i.e. the change persists in the source database (to which PowerSync is connected) before the [`uploadData` call](/usage/installation/client-side-setup/integrating-with-your-backend) completes.
...
For Postgres, this involves adding the table to the [PowerSync logical replication publication](/usage/installation/database-setup), for example:
