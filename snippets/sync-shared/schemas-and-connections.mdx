{/* Shared body: rendered by sync/advanced/schemas-and-connections.mdx (Sync Streams section) and sync/rules/schemas-and-connections.mdx (Sync Rules (Legacy) section). Keep the content valid for both engines. */}
...
It will be possible for each connection to be configured with a "tag", to distinguish these connections in sync queries. The same tag may be used for multiple connections (if the schema is the same in each).
