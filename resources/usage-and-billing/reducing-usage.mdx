...
With [incremental reprocessing](/sync/advanced/incremental-reprocessing) (Beta; currently requires a MongoDB source database and MongoDB bucket storage), changing one stream does not make clients download every other stream again. Clients keep their existing data for unchanged streams and download data only for new or changed streams.
