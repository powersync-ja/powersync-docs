...
<Note>
  **Rust**: The snippet shows the Tokio runtime. You must call `PowerSyncEnvironment::powersync_auto_extension()` before using the SDK and spawn async tasks with `db.async_tasks().spawn_with_tokio()` (or `spawn_with` for other runtimes) before connecting. See [Rust SDK reference — Instantiate the PowerSync Database](/client-sdks/reference/rust#2-instantiate-the-powersync-database) for in-memory pools, smol, or custom runtimes.
</Note>
