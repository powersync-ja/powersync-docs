...
The PowerSync Service connects directly to the backend source database and uses a change stream to hydrate dynamic data partitions, called [sync buckets](/sync/rules/buckets). Sync buckets are used to partition data according to the configured Sync Rules. (In most use cases, only a subset of data is required in a client's database and not a copy of the entire source database.)

<Frame>
  <img src="/images/architecture/powersync-docs-diagram-client-architecture-001.png" />
</Frame>

The local SQLite database embedded in the PowerSync SDK is automatically kept in sync with the backend source database, based on the [Sync Rules](/sync/rules/overview) configured by the developer:
