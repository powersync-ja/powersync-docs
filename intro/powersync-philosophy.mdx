...
description: "PowerSync's vision is that local-first architecture should be easier to implement than cloud-first and deliver a better end-user experience."
---
...
The app is always [fast and responsive](https://www.powersync.com/blog/local-first-is-a-big-deal-especially-for-the-web#to-the-user-everything-feels-instant-no-loading-spinners). There is no need to wait for network requests.
...
[SQLite is _fast_](https://www.powersync.com/blog/sqlite-optimizations-for-ultra-high-performance). It can perform tens of thousands of updates per second, even faster reads, with support for concurrent reads. Once you get to filtering through thousands of rows in queries, indexes keep the queries fast.
