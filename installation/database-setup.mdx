...
When activity on the source database is low, there can be extended periods of inactivity in the binlog.
This can potentially cause the replication connection to be closed by the MySQL server, which will result in PowerSync being unable to read the binlog until it reconnects.
To get around this, PowerSync can optionally be configured to regularly update an entry in a keepalive table in the source database.
This ensures a consistent stream of updates in the binlog, which helps keep the connection alive.

If the PowerSync MySQL user also has the **CREATE**, **DROP**, **INSERT** and **UPDATE** privileges, PowerSync can automatically create and manage the keepalive table in the source database.
```sql
-- Grant additional privileges on the source database
GRANT CREATE, DROP, INSERT, UPDATE ON <source_database>.* TO 'repl_user'@'%';

-- Apply changes
FLUSH PRIVILEGES;
```
The keepalive table can also be manually created, allowing the DB user to make do with more restrictive privileges.
```sql
CREATE TABLE IF NOT EXISTS
	<source_database>.powersync_keepalive(
	id INT PRIMARY KEY DEFAULT 1,
	last_ping TIMESTAMP(3) DEFAULT CURRENT_TIMESTAMP(3)
);

-- Grant additional privileges for the keepalive table
GRANT SELECT, INSERT, UPDATE ON <source_database>.powersync_keepalive TO 'repl_user'@'%';

-- Apply changes
FLUSH PRIVILEGES;
```
If PowerSync is unable to create the keepalive table, and it doesn't exist, a warning is logged and the keepalive feature is disabled.
