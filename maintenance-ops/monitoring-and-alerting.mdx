...
* **Alert Notifications**: Set up [Email notifications](#email-rules) or [Webhooks](#webhooks) to report events (like issue or usage alerts) to external systems \*
...
View time-series and aggregated usage data for your PowerSync instance(s), including storage size, concurrent clients, and synced data and operations. This data lets you monitor activity, spot patterns or spikes, and budget while tracking your position within our [Cloud pricing plans](https://www.powersync.com/pricing).
...
Use instance logs to investigate replication, client sync, and deployment issues. Choose a log type based on the activity you need to inspect:

* **Sync & API logs**: Client sync sessions and other API requests.

* **Replicator logs**: Data moving from your source database into the PowerSync Service.

* **Compact logs**: Automatic and manually triggered [compacting jobs](/maintenance-ops/compacting-buckets).

* **Migration logs**: Storage preparation during a deploy. Check these if a deploy fails at that step.

See the [Log Reference](/debugging/log-reference) for message meanings and structured fields. Log retention depends on your plan; see [Feature Availability](#summary-of-feature-availability-by-powersync-cloud-plan).
...
* **Log Type**: Select one of the log types described above.
...
Hover over a log entry to reveal a button that copies the full log event as JSON.
...
Find a `Sync stream started` or `Sync stream complete` entry and copy its `rid` (request ID). Search for that ID to follow the session:

```
rid:<request-id>
```

Use the [Session Lifecycle](/debugging/log-reference#session-lifecycle) and [Session Fields](/debugging/log-reference#session-fields) reference to interpret the results.

### Replication Flush Entries

To investigate replication throughput, select Replicator logs and search for `Flushed`. Each entry reports a batch of writes to bucket storage. See [Flushed Batches](/debugging/log-reference#flushed-batches) for message and field meanings.
...
<Warning>
  **Important: Set Up Notification Rules**
  ...
  Creating an issue alert only defines *what* to monitor. To actually receive notifications when alerts trigger, you must also set up [Email Rules](#email-rules) or [Webhooks](#webhooks) and configure them to notify for "Issue alert state change" events. See the [Alert Notifications](#alert-notifications) section below.
</Warning>
...
<Warning>
  **Important: Set Up Notification Rules**
  ...
  Creating a usage alert only defines *what* to monitor. To actually receive notifications when alerts trigger, you must also set up [Email Rules](#email-rules) or [Webhooks](#webhooks) and configure them to notify for "Usage alert state change" events. See the [Alert Notifications](#alert-notifications) section below.
</Warning>
