...
This is the "[Sync Rule](/usage/sync-rules)" that describes buckets. In Sync Rules (legacy), this specifies the name, parameter query(ies), and data queries.

Each bucket definition describes a set of buckets using SQL-like queries.

<Note>
  [Sync Streams](/usage/sync-streams.md) use a different approach with unified queries instead of separate parameter and data queries.
</Note>
