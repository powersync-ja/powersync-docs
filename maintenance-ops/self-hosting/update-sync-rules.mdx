...
1. **CLI** — Edit your config and apply with `powersync docker reset` (recommended if using the CLI)
...
<Tip>
  Run `powersync validate` in the CLIbefore deploying to catch errors in your sync config without applying changes. Works with any running PowerSync instance.
</Tip>
