{/* Shared body: rendered by sync/advanced/reducing-bucket-count.mdx (Sync Streams section) and sync/rules/reducing-bucket-count.mdx (Sync Rules (Legacy) section). Keep the content valid for both engines. */}
