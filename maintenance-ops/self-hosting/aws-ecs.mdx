...
The task definitions below allocate **2 vCPU and 2GB memory** per container, which is a good starting point. You can adjust resources based on your workload — see [Deployment Architecture](/maintenance-ops/self-hosting/deployment-architecture) for scaling guidance (recommended minimum: 1 vCPU, 1GB memory).
