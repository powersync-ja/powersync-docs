<Steps>
  ...
  <Step title="Step 3: Generate a development token">
    ...
    3. Generate a new token by running the following command in the `test-client` directory with your updated `config.yaml` file:
      ...
      - If you're syncing data without parameters (i.e. data shared between all users), you can use any value (e.g., `test-user`)
      - If you're using Sync Streams/Rules that filter data by user, use a user ID that matches a user in your database (this will be used as `auth.user_id()` in Sync Streams or `request.user_id()` in Sync Rules)
  </Step> 
</Steps>
