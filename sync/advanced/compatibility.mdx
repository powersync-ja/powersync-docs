...
When you omit `storage_version`, the PowerSync Service uses its default, which is version 2 in v1.26.0. Self-hosted deployments can change the default with `storage.default_storage_version` in `service.yaml`. On PowerSync Cloud, PowerSync manages the default.
