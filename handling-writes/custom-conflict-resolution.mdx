...
3. **Clients download updates** - Based on their Sync Streams
...
- Don't include the `field_changes` table in your Sync Streams
...
**Tradeoff:** Can't show detailed server-side error messages (unless the server writes to a separate errors table that *is* in Sync Streams). No long-term history on the client.
