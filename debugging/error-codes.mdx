...
- **PSYNC_S1146**:
  ...
  The replication slot was invalidated by PostgreSQL, typically because WAL retention exceeded `max_slot_wal_keep_size` during a long-running snapshot. Increase `max_slot_wal_keep_size` on the source database and redeploy Sync Streams/Sync Rules to trigger a fresh snapshot.
  
  Other causes: `rows_removed` (catalog rows needed by the slot were removed), `wal_level_insufficient`, or `idle_timeout` (PostgreSQL 18+).
