...
PowerSync's client SDKs provide APIs to allow applications to track sync status at different priority levels. This includes:

- `waitForFirstSync(priority: int)` to allow waiting for a specific priority level.
- `SyncStatus.statusForPriority(priority: int)` providing details on the last sync time per priority level.

Developers can leverage these methods to ensure critical data is available before proceeding with UI updates or background processing.