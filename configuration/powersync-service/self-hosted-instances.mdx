...
1. A connection to your [source database](/configuration/source-db/setup), which PowerSync replicates data from.
...
3. [Client authentication](/configuration/auth/overview). When getting started, you can use temporary [development tokens](/configuration/auth/development-tokens) instead of setting up a full auth provider.
...
<Card title="Example service.yaml" icon="github" horizontal href="https://github.com/powersync-ja/self-host-demo/blob/main/demos/nodejs/powersync/service.yaml">
  The config used by the Node.js (Postgres) demo in `self-host-demo`. Similar examples exist for MongoDB, MySQL, and SQL Server. Use the demos as a working reference for your own setup.
</Card>
...
Substituted values are strings. For options that expect a number or boolean, append `::number` or `::boolean` to the variable name to cast the value. A `::boolean` cast accepts `true` or `false`.

```yaml service.yaml
storage:
  max_pool_size: !env PS_STORAGE_POOL_SIZE::number

telemetry:
  disable_telemetry_sharing: !env PS_DISABLE_TELEMETRY::boolean
```
...
<Note>
  Hosted Supabase only allows direct database connections over IPv6, and PowerSync cannot connect using the connection pooler. You can:

  - Enable IPv6 for Docker as per [the Docker docs](https://docs.docker.com/config/daemon/ipv6/).
  - Enable IPv4 on your Supabase project.
...
<AccordionGroup>
  <Accordion title="Postgres connection options">
    ...
    <ResponseField name="snapshot_socket_timeout" type="number" default="30">
      Idle timeout in seconds for snapshot connection sockets. If the storage database cannot keep up during the initial snapshot, a storage flush can stall the snapshot for longer than this timeout, which closes the source connection mid-snapshot. This appears in the Service logs as `Socket timed out` errors during the initial snapshot. Increase the timeout if you see these errors. Available since Service version 1.26.0.
    </ResponseField>

    <ResponseField name="heartbeat_interval_seconds" type="number" default="60">
      Interval in seconds between source connection heartbeats. Must be between 5 and 60. Available since Service version 1.24.0.
    </ResponseField>
  </Accordion>
  ...
  <Accordion title="SQL Server connection options">
    ...
    <ResponseField name="schema" type="string" default="dbo">
      The database schema to replicate from.
    </ResponseField>

    <ResponseField name="cacert" type="string">
      CA certificate content in PEM format. Available since Service version 1.25.0.
    </ResponseField>

    <ResponseField name="tls_servername" type="string">
      Use a servername for TLS that is different from `hostname`. Available since Service version 1.25.0.
    </ResponseField>
  ...
  <Accordion title="Convex connection options">
    <Note>The Convex replicator is currently released as an [experimental feature](/resources/feature-status) and is available since Service version v1.22.0. See [Convex source database setup](/configuration/source-db/setup#convex).</Note>
...
<ResponseField name="uri" type="string" required>
  Connection URI in the format `mongodb://` or `mongodb+srv://`.
</ResponseField>
...
<ResponseField name="username" type="string">
  Database username. Defaults to the username in the URI. See **Required Permissions** below for the roles the user needs.
</ResponseField>
...
<ResponseField name="clear_batch_throttle_rate" type="number" default="0.2">
  Throttles the clearing of old bucket data after deploying a new sync configuration, by pausing between batches. The pause is proportional to the previous batch duration. Increase this to reduce the impact of clear operations on the storage cluster, or use `0` to clear as fast as possible. Must be between 0 and 20. Available since Service version 1.24.0.
</ResponseField>
...
<ResponseField name="chunk_compaction_concurrency" type="number">
  ...
  Chunk-merge compaction runs after initial replication, before the new Sync Config becomes active, and as part of scheduled [compact](/maintenance-ops/compacting-buckets) runs. With object storage, processing one bucket at a time spends most of the run waiting on requests, so a higher value shortens the delay before activation. All compaction jobs in a Service process share this limit. Available since Service version 1.26.1.
</ResponseField>
...
<ResponseField name="object_storage" type="object">
  ...
  <Expandable title="properties">
    ...
    <ResponseField name="concurrency_limit" type="number" default="64">
      Maximum number of concurrent object storage requests. Service versions before 1.26.1 default to `16`.
    </ResponseField>
    <ResponseField name="defaults_mode" type="string" default="standard">
      ...
      When this option is unset, the Service reads the `AWS_DEFAULTS_MODE` environment variable, then falls back to `standard`. Setting `defaults_mode` in an AWS shared config file has no effect. Available since Service version 1.26.0.
    </ResponseField>
    <ResponseField name="inline_threshold_bytes" type="number" default="16384">
...
No access beyond the storage database is required. The `readWrite` role covers regular operation, including creating and dropping collections and indexes. The `dbAdmin` role is additionally needed to drop the database when an instance is torn down.
...
Postgres storage accepts the same connection options as a [Postgres replication connection](#replication): `uri`, `hostname`, `port`, `username`, `password`, `database`, `sslmode`, `cacert`, `client_certificate`, `client_private_key`, `tls_servername` and `connect_timeout`. In addition, batch limits can be tuned:
...
<ResponseField name="parameters" type="object">
  ...
  <Expandable title="properties">
    ...
    <ResponseField name="checkpoint_request_retention_minutes" type="number" default="60">
      Number of minutes to keep client-requested write checkpoint records. Expired records are removed by the compact job. Must be a positive integer. Available since Service version 1.24.0.
    </ResponseField>
    <ResponseField name="bucket_count_cache_ttl_minutes" type="number" default="60">
      How long to keep cached bucket counts before refreshing them, in minutes. Bucket counts may be affected by compacting. Available since Service version 1.23.2.
    </ResponseField>
  </Expandable>
</ResponseField>
