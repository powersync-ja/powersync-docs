...
- **Large initial snapshots need storage v3 or later.** DocumentDB retains only a limited amount of change-feed history. On storage versions 1 and 2, a large or busy source can age its resume position out of that window before the initial snapshot finishes, causing replication to restart and loop. Storage version 3 and later avoid this by streaming during the snapshot. See [Storage Version](/sync/advanced/compatibility#storage-version) for how storage versions work and how to opt in.
