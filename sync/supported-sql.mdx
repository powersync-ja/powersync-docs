...
- **Global CTE names must not shadow source table names.** If a global CTE has the same name as a database table or collection, PowerSync reports a validation error and refuses to apply the config. Stream-level CTE names are not subject to this restriction.
