...
config:
  edition: 3
  storage_version: 2 # version 2 (stable) and 3 (experimental) are currently supported
...
2. Experiments: Opt into an experimental storage version in non-production environments.
...
- Odd numbers (for example `3`) denote experimental formats. The layout may change without notice and support may be removed in a future release. Use odd versions only for testing, not production.
