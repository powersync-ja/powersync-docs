...
<Tabs>
  ...
  <Tab title="Self-Hosted">
    ...
    **Docker plugin (CLI)**
    ...
    If you're connecting to an external database (not the Docker-managed Postgres), update the `replication.connections` section in `powersync/service.yaml` with your connection URI — same format as the examples above.
  </Tab>
</Tabs>
