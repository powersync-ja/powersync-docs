---
title: "Supported SQL"
description: "Reference for SQL syntax, operators, built-in functions, and type casting supported in Sync Streams/Sync Rules queries."
sidebarTitle: "Guide"
---
...
<AccordionGroup>
  ...
  These functions return metadata about the table a row comes from, rather than operating on a value. Prefix each call with the name or alias of a table in the query's `FROM` clause, the same way you would qualify a column reference: with `FROM "todos_%" AS todos`, write `todos.table_suffix()`. The prefix must refer to an actual table; using a subquery alias as the prefix is not supported. The functions take no arguments, and can appear in `WHERE` clauses, in selected columns, and inside subqueries.
  ...
  <Info>Supported in Sync Streams only, with `edition: 3` and PowerSync Service v1.24.0 or later.</Info>
  </Accordion>
