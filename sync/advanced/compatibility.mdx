...
When you omit `storage_version`, the PowerSync Service uses its configured default. Self-hosted MongoDB bucket storage defaults to version 2 unless you set `storage.default_storage_version` in `service.yaml`. PowerSync Cloud manages this setting for you.
...
| `4` | MongoDB | Can improve initial sync performance with S3 bucket storage and limit reprocessing after Sync Config changes. Postgres bucket storage does not support version 4. |

A stable format means future Service versions can continue reading data written with that version until it is officially deprecated. This is separate from feature maturity: incremental reprocessing and S3 bucket storage are currently in [Beta](/resources/feature-status).

Odd-numbered versions, such as `3`, are experimental: their layout can change without notice and support can be removed in a future release. If you previously set `storage_version: 3`, deploy with version `4`, which uses the same format marked stable.
