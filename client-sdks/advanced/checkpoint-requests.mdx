...
The managed flow assumes that `uploadData()` returns only after your backend commits the uploaded changes to the source database. If your backend queues uploads for later processing, use the same source-side behavior described in [Custom Write Checkpoints](/handling-writes/custom-write-checkpoints).
