...
3. Back in the PowerSync dashboard, paste the connection string into the connection's **URI** field. PowerSync will automatically parse this URI to populate the database connection details.
...
8. Click **Next**.
9. PowerSync will detect the Supabase connection and prompt you to enable Supabase auth. To enable it, copy your JWT Secret from the Supabase Dashboard's API settings and paste it into the form:
    <img src="/images/installation/overview-supabase-auth.png" width="60%" />
10. Click **Enable Supabase auth** to finalize your connections settings.

PowerSync will now create an isolated cloud environment for your instance. This process typically takes a minute or two.
