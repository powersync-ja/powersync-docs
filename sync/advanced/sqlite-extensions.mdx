...
This document provides describes how custom extensions can be used on the different PowerSync SDKs.
...
can be used to load custom SQLite extensions for PowerSync.
...
For `sqlite-vec`, depending on [this crate](https://crates.io/crates/sqlite-vec) and calling
...
with the `_init` function of the extension (useful for statically linked extensions) or use 
