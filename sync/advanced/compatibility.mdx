...
- Delay a storage upgrade. When the default moves to a newer version, pin `storage_version` to the version your data already uses. This keeps later Sync Config deployments on that format. Remove the pin when you are ready for the new format.
...
All PowerSync Cloud instances use MongoDB bucket storage, so they are compatible with all available storage versions. Self-hosted instances with Postgres bucket storage can use versions 1 and 2 only.
...
Streams are compiled into buckets, and rows to sync are assigned to those buckets. When you run a full defragmentation or
redeploy your Sync Config, the same bucket identifiers are re-used when processing data again.
...
This fixes the `json_extract` functions as well as the `->` and `->>` operators in stream queries to behave similar
