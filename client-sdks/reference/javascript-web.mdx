...
This makes it unsuitable for apps that need to work offline, but it can be useful for development. The VFS is also faster than
all other single-threaded setups (both IndexedDB and OPFS except the write-ahead VFS), making it useful for online-only apps
with small data sizes and very frequent queries.
