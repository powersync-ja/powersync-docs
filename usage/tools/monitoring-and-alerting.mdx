...
<Tabs>
  <Tab title="JavaScript/React Native/Node.js">
    ```javascript
  ...
  <Tab title="Dart/Flutter">
    ...
    final powerSync = PowerSyncDatabase(
      schema: AppSchema(),
      database: DatabaseFile('powersync.db'),
    );
  ...
  <Tab title="Swift">
    ...
    let powerSync = PowerSyncDatabase(
      schema: AppSchema(),
      database: DatabaseFile(filename: "powersync.db")
    )

    // Set custom metadata for the connection
    try await powerSync.setMetadata([
      "app_version": "1.2.3",
      "feature_flag": "new_sync_flow"
    ])

    // Clear metadata by setting it to nil
    try await powerSync.setMetadata(nil)
    ```
  </Tab>
  ...
  <Tab title=".NET">
    ...
    var powerSync = new PowerSyncDatabase(
      schema: new AppSchema(),
      database: new DatabaseFile("powersync.db")
    );

    // Set custom metadata for the connection
    await powerSync.SetMetadata(new Dictionary<string, string>
    {
      { "app_version", "1.2.3" },
      { "feature_flag", "new_sync_flow" }
    });
    ...
    await powerSync.SetMetadata(null);
    ```
  </Tab>
</Tabs>
