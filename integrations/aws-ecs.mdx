...
flowchart TD
    subgraph vpc["AWS VPC"]
        ...
        subgraph private["Private Subnets"]
            ecs(["ECS Fargate<br/>PowerSync Service"])
        end

        alb ~~~ ecs
    end

    ecs --> db(["Source Database<br/>& Bucket Storage"])
    internet([Internet]) --> alb
    alb --> ecs
    ecs -.-> nat
    nat -.-> internet

    style vpc fill:transparent,stroke:#0066cc,stroke-width:2px
    style public fill:transparent,stroke:#ff9900,stroke-width:2px
    style private fill:transparent,stroke:#666,stroke-width:2px
    style alb fill:transparent,stroke:#e67300,stroke-width:2px
```
...
PowerSync requires MongoDB or PostgreSQL 14+ for bucket storage. Choose one:
...
replication:
  connections:
    - type: postgresql  # or mongodb, mysql, sqlserver
...
# Allow ECS to reach databases
aws ec2 authorize-security-group-ingress \
  --group-id sg-mongodb-xxxxx \
  --protocol tcp --port 27017 --source-group sg-ecs-xxxxx
```

### NAT Gateway (Required)

ECS tasks need outbound internet for ECR and JWKS URLs:
...
```bash
# Store config (base64 encoded)
aws secretsmanager create-secret \
  --name powersync/config-b64 \
  ...
  "containerDefinitions": [
    {
      ...
      "secrets": [
        {"name": "POWERSYNC_CONFIG_B64", "valueFrom": "arn:aws:secretsmanager:YOUR_REGION:YOUR_ACCOUNT_ID:secret:powersync/config-b64"},
...
<Note>
Pin to a specific version for production. Check [Docker Hub](https://hub.docker.com/r/journeyapps/powersync-service/tags) for latest releases.
</Note>
