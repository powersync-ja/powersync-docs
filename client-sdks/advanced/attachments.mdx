...
In the JavaScript SDKs, this adapter receives the full file as one in-memory buffer. On React Native and Node.js, you can replace the adapter with a streaming transport to move large files without this buffer. See [Transferring Large Files Without Buffering](#transferring-large-files-without-buffering).
