...
description: "Reduce storage and sync overhead by compacting and defragmenting data."
---
...
With [incremental reprocessing](/sync/advanced/incremental-reprocessing) (Beta; currently requires a MongoDB source database and MongoDB bucket storage), changing one stream rebuilds only the sync data selected by that stream. This reduces reprocessing during the deployment and prevents clients from downloading unchanged streams again. Buckets for unchanged streams also keep their operation history, so a deployment no longer acts like defragmenting every bucket.

Without incremental reprocessing, a deployment reads all data selected by the Sync Config and creates new buckets for it. Clients then download that data again. Do not redeploy a Sync Config for routine history cleanup. Use compacting or a deliberate defragmentation instead.
