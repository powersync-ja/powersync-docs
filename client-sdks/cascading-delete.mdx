...
description: "Implement cascading deletes in client-side SQLite using triggers and upload queue handlers."
keywords: ["data", "cascade", "delete"]
---

PowerSync [uses SQLite views](/architecture/client-architecture#client-side-schema-and-sqlite-database-structure) instead of standard tables, so SQLite features like foreign key constraints and cascading deletes are not available.
...
2. Triggers - Create triggers on the [internal tables](/architecture/client-architecture#client-side-schema-and-sqlite-database-structure) (more complex, but more automatic)
   <Note>
