...
description: "Design Sync Rules to organize data into global and user-filtered buckets."
---

<Info>
Sync Rules are deprecated. For the Sync Streams equivalent, see the [Sync Streams Quickstart](/sync/streams/quickstart).
</Info>
...
* If there's data that you want to filter by user, so that different users/clients get different subsets of data, you can add bucket definitions to your Sync Rules specifically for that purpose, using the `user_id` _Authentication Parameter_ that comes from the JWT.
