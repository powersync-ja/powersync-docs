...
Sync Streams will ultimately replace the current Sync Rules system. They are currently in a closed alpha release with a limited set of customers for validation and soak testing. We are actively seeking your feedback on their performance for your use cases, the developer experience, missing capabilities, and potential optimizations.
...
* v1.15.0 of the PowerSync Service (currently available on the `Next` channel)
...
* Each Sync Stream has a `ttl` (time-to-live). After you call `unsubscribe()`, or when the page/app closes, the stream keeps syncing for the `ttl` duration, enabling caching-like behavior. Each SDK lets you specify the `ttl`, or ignore the `ttl` and delete the data as soon as possible.
