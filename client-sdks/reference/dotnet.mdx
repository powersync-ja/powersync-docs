...
* Signed up for a PowerSync Cloud account or are self-hosting PowerSync ([see docs](/intro/setup-guide#1-set-up-powersync-service-instance)).
* [Configured your backend source database](/intro/setup-guide#2-configure-your-source-database) and connected it to your PowerSync instance.
...
When using [bucket priorities](/sync/advanced/prioritized-sync), you can access priority-specific sync status information using [PriorityStatusEntries](https://github.com/powersync-ja/powersync-dotnet/blob/main/PowerSync/PowerSync.Common/DB/Crud/SyncStatus.cs) and [StatusForPriority](https://github.com/powersync-ja/powersync-dotnet/blob/main/PowerSync/PowerSync.Common/DB/Crud/SyncStatus.cs):
