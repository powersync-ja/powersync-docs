...
With [incremental reprocessing](/sync/advanced/incremental-reprocessing#custom-checkpoint-events), changing another stream does not rebuild this checkpoint data. If you change the event query itself, clients continue using checkpoints from the current query while PowerSync prepares checkpoint data from the new query.
