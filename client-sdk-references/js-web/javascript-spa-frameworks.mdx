...
The PowerSync [JavaScript/Web SDK](../js-web) is compatible with popular Single-Page Application (SPA) frameworks like React, Vue, Angular, and Svelte. For [React](javascript-spa-frameworks#react-hooks) and [Vue](javascript-spa-frameworks#vue-composables) specifically, wrapper packages are available to support reactivity and live queries, making it easier for developers to leverage PowerSync's features.

PowerSync also integrates with [TanStack Query for React](javascript-spa-frameworks.md#powersync-less-than-greater-than-tanstack-alpha) (details below). This integration provides a wide range of developer tools and paves the way for future live query support in other frameworks.

### Which package should I choose for queries?

For React or Native Native apps:

* The [`@powersync/react`](javascript-spa-frameworks.md#react-hooks) package is best for most basic use cases, especially when you only need reactive queries with loading and error states.
* For more advanced scenarios, such as query caching and pagination, TanStack is a powerful solution. The [`@powersync/tanstack-react-query`](javascript-spa-frameworks.md#powersync-less-than-greater-than-tanstack-alpha) package extends the `useQuery` hook from `@powersync/react` and adds functionality from [TanStack Query](https://tanstack.com/query/latest/docs/framework/react/overview), making it a better fit for advanced use cases or performance-optimized apps.

If you have a Vue app, use the Vue-specific package: [`@powersync/vue`](javascript-spa-frameworks.md#vue-composables).
...
<Card
  title="React hooks for PowerSync | PowerSync JS SDK Docs"
  icon={<img src="/images/logo.png"/>}
  href="https://powersync-ja.github.io/powersync-js/react-sdk"
  horizontal
/>

## PowerSync \<\> TanStack (Alpha)

PowerSync integrates with [TanStack Query (formerly React Query)](https://tanstack.com/query/latest/docs/framework/react/overview) through the `@powersync/tanstack-react-query` package.
...
<Card
  title="TanStack React Query | PowerSync JS SDK Docs"
  icon={<img src="/images/logo.png"/>}
