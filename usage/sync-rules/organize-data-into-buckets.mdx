...
<Tabs>
  <Tab title="Sync Streams">
    With Sync Streams, you define streams using queries (and optionally subqueries) that clients can subscribe to with parameters:
    ...
    Sync Streams use a unified query syntax — there are no separate "data queries" or "parameter queries". Clients subscribe to streams with parameters as needed. See the [Sync Streams documentation](/usage/sync-streams.md) for client-side syntax.
  </Tab>
  
  <Tab title="Sync Rules (legacy)">
    With Sync Rules, this is defined using two types of queries:
