...
See [Monitoring and Alerting](monitoring-and-alerting.mdx).
...
The JavaScript SDKs ([React Native](/client-sdk-references/react-native-and-expo), [web](/client-sdk-references/js-web)) also log the contents of sync bucket changes to `console.debug` if verbose logging is enabled. This should log which PUT/PATCH/DELETE operations have been applied from the server.
...
Our client SDKs support logging to troubleshoot issues. Logging is enabled by default in our [Flutter SDK](/client-sdk-references/flutter/usage-examples#using-logging-to-troubleshoot-issues). See the following usage examples for enabling logging in our SDKs:

* [React Native](/client-sdk-references/react-native-and-expo/usage-examples#using-logging-to-troubleshoot-issues)
* [JS Web](/client-sdk-references/js-web/usage-examples#logging)
* [Kotlin Multiplatform](/client-sdk-references/kotlin-multiplatform/usage-examples#using-logging-to-troubleshoot-issues)
...
Enabling the `debugMode` flag in the [JavaScript Web SDK](/client-sdk-references/js-web) logs all SQL queries on the Performance timeline in Chrome's Developer Tools (after recording). This can help identify slow-running queries.
<Frame>
  <img src="/images/resources-1.avif"/>
</Frame>
