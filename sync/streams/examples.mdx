...
Both queries produce the same result. You can express these nested relationships directly, without denormalizing your database or adding redundant foreign keys. Keep in mind that the query shape determines how many buckets each user syncs. Joining or filtering through parent tables creates one bucket per matching child row, which can grow large at scale. See [Bucket Count and Limits](/sync/streams/bucket-count) to plan for this. Clients auto-subscribe to `org_tasks` when they connect, with no client-side subscription call needed.
