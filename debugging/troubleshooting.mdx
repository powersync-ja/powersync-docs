...
The JavaScript SDKs ([React Native](/client-sdks/guides/react-native-and-expo), [web](/client-sdks/guides/javascript-web)) also log the contents of sync bucket changes to `console.debug` if verbose logging is enabled. This should log which `PUT`/`PATCH`/`DELETE` operations have been applied from the server.
...
Enabling the `debugMode` flag in the [Web SDK](/client-sdks/guides/javascript-web) logs all SQL queries on the Performance timeline in Chrome's Developer Tools (after recording). This can help identify slow-running queries.
<Frame>
