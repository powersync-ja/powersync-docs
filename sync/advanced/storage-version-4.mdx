...
| | Source database | PowerSync Cloud (Beta) | Self-hosted (Beta) |
| --- | --- | --- | --- |
| Storage version 4 | Any | Free plan: automatic. Other plans: [opt in](#opt-in). | [Opt in](#opt-in) |
| Incremental reprocessing | MongoDB | Included with version 4 | Included with version 4 |
| S3 object storage | Any | Enabled per instance by PowerSync on request | [Set up S3 object storage](#self-hosted-s3-setup) |
...
S3 object storage requires storage version 4. It is compatible with all PowerSync Cloud instances.
