...
- Storing files locally: Attachments are stored locally on the device in a structured way, linked to specific database records.
- Tracking sync state: A local attachment queue manages uploads, downloads, and retries, ensuring local files stay in sync with remote storage.
- Syncing metadata and files: It uses PowerSync to synchronize both attachment metadata and the actual files between local and remote storage.
- Handling offline-first workflows: Files are accessible offline, with changes queued and synced when connectivity is restored.
- Providing a file operations API: Offers methods to add, remove, and retrieve attachments, ensuring consistency across local and remote storage.
