...
<Info>
Sync rules defined in your `powersync.yaml` configuration file take **priority** and override any rules deployed via the API.
...
Sync rules deployed via the API are overwritten if sync rules are later added to `powersync.yaml` and the service is restarted.
