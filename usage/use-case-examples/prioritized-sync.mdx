...
2. `SyncStatus.statusInPriority(priority: int)` This method provides details on the last sync state and time per priority level.

## Example
Using the above we can render a lists component only once the user's lists (with priority 1) have completed syncing, else show a loading spinner:
