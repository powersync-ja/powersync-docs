...
description: "Implement custom JWT-based authentication for PowerSync using any auth provider."
---
...
1. The JWT must be signed using a key in the JWKS ([Option 1](#option-1-asymmetric-jwts-—-using-jwks-recommended)) or the HS256 key ([Option 2](#option-2-symmetric-jwts-—-using-hs256))
...
4. Additional fields can be added which can be referenced in Sync Streams (as [`auth.parameters()`](/sync/streams/parameters#auth-parameters)) or Sync Rules [parameter queries](/sync/rules/parameter-queries).
