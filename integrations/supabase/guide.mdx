...
Run the below SQL statements in your **Supabase SQL Editor**. Creating the tables, granting Data API access, enabling RLS, and adding policies are treated as a single unit so the tables are never reachable in an insecure state:
