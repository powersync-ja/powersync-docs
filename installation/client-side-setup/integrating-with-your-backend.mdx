...
|---------|----------|-------------|
| **Uploading writes to your backend:** | `uploadData()` | The PowerSync Client SDK automatically calls this function to upload client-side write operations to your backend. Whenever you write to the client-side SQLite database, those writes are also automatically placed into an _upload queue_ by the Client SDK, and the Client SDK processes the entries in the upload queue by calling `uploadData()`. The Client SDK automatically handles retries in the case of failures. See [Writing Client Changes](/installation/app-backend-setup/writing-client-changes) for considerations on the backend implementation. |
