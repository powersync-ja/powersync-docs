...
<Tabs>
  <Tab title="PowerSync Cloud">
    ...
    5. Enter a user ID:
       - If you're only syncing data without parameters (i.e. data shared between all users), you can enter any value (e.g., `test-user`) since all data syncs to all users
       - If you're using any Sync Streams / Sync Rules that filter data by user, enter a user ID that matches a user in your database. The user ID will be used as `auth.user_id()` in Sync Streams or `request.user_id()` in Sync Rules.
