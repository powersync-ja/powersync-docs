...
<Tabs>
      ...
      <Tab title="Self-Hosted">
        ...
        # 3. Run MongoDB for sync bucket storage
        docker run -d \
          --name powersync-mongo \
        ...
        # 5. Create config.yaml (see below)

        # 6. Run PowerSync Service
        docker run -d \
        --name powersync \
        --network powersync-network \
        -p 8080:80 \
        ...
        **Basic `config.yaml` structure:**
        ...
        replication:
          connections:
            - type: postgresql # or mongodb, mysql, mssql
              uri: postgresql://postgres:mypassword@pg-db:5432/postgres # The connection URI or individual parameters can be specified.

        # Connection settings for sync bucket storage
        storage:
          type: mongodb
          uri: mongodb://powersync-mongo:27017/powersync
        ...
        <Tip>
          **Learn More**
          * [Sync Bucket Storage](/configuration/powersync-service/sync-bucket-storage)
          * [Self-Hosted Service Configuration](/configuration/powersync-service/self-hosted-service)
        </Tip>
      </Tab>
    </Tabs>
  ...
  <Tab title="Self-Hosted">
    ...
    <CodeGroup>
      ...
      replication:
        connections:
          - type: postgresql
            uri: postgresql://powersync_role:password@host:5432/database # The connection URI or individual parameters can be specified.
...
Connect your client-side PowerSync database to the PowerSync Service instance you created in [step 1](#1-set-up-powersync-service-instance) by defining a _backend connector_ and calling `connect()`. The backend connector handles authentication and uploading writes to your backend.
...
# 11. Read and Write Data (Using SQLite)
...
- **Configure & Integrate Your Backend Application**: Set up your backend to handle writes uploaded from clients.
