...
1. An HTTP cliemt (using traits from the `http-client` crate). We recommend enabling the `curl_client` feature
   on that crate and then using an `IsahcClient` with PowerSync. The `H1Client` is known not to work with
   PowerSync because it can't cancel response streams properly.
...
<Tabs>
<Tab title="File-based database">

To allow readers and writers to progress concurrently, use `ConnectionPool::wrap_connections`.
The SDK will automatically transition the database into WAL mode.
...
fn open_pool() -> rusqlite::Result<ConnectionPool> {
...
Next, create a database and start asynchronous tasks:
...
To avoid spawning tasks dynamically, asynchronous jobs for e.g. the sync client are exposed through
`PowerSyncDatabase::async_tasks()`. There is a utility function to start those tasks in a tokio runtime.
...
To avoid spawning tasks dynamically, asynchronous jobs for e.g. the sync client are exposed through
`PowerSyncDatabase::async_tasks()`. These can be spawned by providing a function turning these futures
into independent tasks, e.g. with `smol::spawn`:
