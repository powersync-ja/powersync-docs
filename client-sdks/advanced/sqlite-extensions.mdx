...
The PowerSync .NET SDK has builtin support for loading custom extensions. To load custom extensions, set the `MDSQLiteOptions.Extensions` field to an array of `SqliteExtension` objects. You can also disable loading the [core PowerSync SQLite extension](https://github.com/powersync-ja/powersync-sqlite-core) by setting `MDSQLiteOptions.LoadPowerSyncExtension` to false. When doing so, it is **your responsibility** to ensure that one of the extensions passed in `MDSQLiteOptions.Extensions` contains a build of the PowerSync core extension. Running the PowerSync .NET SDK without the core extension or with an outdated version of the core extension is **not supported**.
