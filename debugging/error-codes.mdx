...
description: "Reference of PowerSync error codes with explanations and troubleshooting guidance."
...
- **PSYNC_S1120**:
  ...
  See the error details for more information about the underlying query failure.
...
- **PSYNC_S1146**:
  ...
  `idle_timeout` is a Postgres 18+ slot invalidation. In this case, increase `idle_replication_slot_timeout` instead of `max_slot_wal_keep_size`.
...
- **PSYNC_S1307**:
  ...
  Check that the user has the required privileges. See the required permissions for the [MongoDB source database](/configuration/source-db/setup#mongodb-permissions) or the [MongoDB storage database](/configuration/powersync-service/self-hosted-instances#mongodb-storage), depending on which connection reported the error.
...
- **PSYNC_S1345**:
  ...
  To help avoid this error, the Service writes periodic heartbeats to the `_powersync_checkpoints` collection when the change stream has not advanced recently. For self-hosted instances, the heartbeat frequency can be tuned with the `heartbeat_interval_seconds` [connection option](/configuration/powersync-service/self-hosted-instances#replication).
...
- **PSYNC_S1601**:
  ...
  Replication stops and requires a Sync Config deployment. Re-enabling CDC creates a new capture instance, which the existing replication process will not adopt.

- **PSYNC_S1602**:
  A table in the Sync Config is not ready to be replicated.
  
  Possible causes:
  - CDC has not been enabled for the table.
  - The table does not exist in the source database.
  
  For a table that has not been replicated by this deployment before, replication starts once the table exists and has CDC enabled, without a new Sync Config deployment.
  
  For a table this deployment was already replicating, re-enabling CDC is not enough: the new capture instance has a different object ID, so the next attempt fails with `PSYNC_S1601` until the Sync Config is redeployed.

- **PSYNC_S1603**:
  A replicated source table was dropped, renamed, recreated, or no longer matches the source identity selected by the current replication process.
  
  Replication stops because the table may still have unread changes. A Sync Config deployment is required to adopt the replacement. Existing replicated data is retained until then.
...
- **PSYNC_S2302**:
  ...
  2. Sync Streams/Rules have been deployed, but are still busy processing.
  
  View the replicator logs to see if the Sync Streams/Rules are being processed.

- **PSYNC_S2304**:
  Maximum active concurrent clients limit has been reached.

- **PSYNC_S2305**:
  Too many buckets, or too many parameter query results.

  This code covers two limits per active connection, both with a default of 1,000: the number of unique buckets (`Too many buckets`) and the number of parameter query results returned before deduplication (`Too many parameter query results`). Read the message to see which limit you reached, then see [Reducing Bucket Count](/sync/advanced/reducing-bucket-count) to diagnose and resolve this error.
...
- **PSYNC_S4105**:
  ...
  When a Sync Streams/Rules file is configured, the dynamic Sync Streams API is disabled.
