...
Two limits sit behind this error, both with a [default of 1,000](/resources/performance-and-limits):
...
Read the message to see which limit you reached. The fix is different for each.

To diagnose:

- The `PSYNC_S2305` log includes a breakdown of the streams contributing the most. For a parameter-limit error, the last stream listed is the one that ran when the budget hit zero. That stream is not always the cause, so check every stream in the breakdown.
- Checkpoint logs record `buckets` and `param_results` for each connection. Find them in your [instance logs](/maintenance-ops/monitoring-and-alerting).
- The [Sync Diagnostics Client](/tools/diagnostics-client) shows a user's buckets, but it will not load for a user who is already over the limit, since their sync fails first.

To diagnose and resolve this, see [Reducing Bucket Count](/sync/advanced/reducing-bucket-count). For how buckets are counted in the first place, see [Bucket Count](/sync/streams/bucket-count).
