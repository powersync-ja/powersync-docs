...
The [PowerSync Node.js SDK](/client-sdk-references/node) comes in handy in these scenarios, because you can use it to on the server, by Node.js applications.
...
<Info>
    ...
    - You will need to wait for the first sync to complete before deleting the `client_id` key and vacuuming the database. This makde sure all of the data is synced to the database before we proceed.
    ...
    - It's important to note that you will need to use the [`VACUUM INTO`](https://sqlite.org/lang_vacuum.html) command to create a clean, portable SQLite database file. This will help to reduce the size of the database file and improve the performance of the database.
    - This example the upload function is using AWS S3, but you can use any blob storage provider that you prefer.
</Info>
...
- After each run, perform some environment cleanup to avoid disk bloat. This can be done by deleting the pre-seeded SQLite database files after they have been used.

## Client Side Usage

When the client applicaitons boot, before connecting to the PowerSync instance, check if the pre-seeded SQLite database exists in the blob storage. If it does, download it and use. 
Once the database is downloaded, insert a new client_id key into the ps_kv table.
