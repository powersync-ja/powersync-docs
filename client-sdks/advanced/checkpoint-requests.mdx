...
<Warning>
Checkpoint requests are an alpha API and may change. Support requires PowerSync Service version 1.24.0 or later. Checkpoint requests are not currently available for the Rust SDK.
</Warning>
...
```cs .NET
Task RefreshLocalData(CancellationToken cancellationToken = default)
{
    var checkpoint = await database.RequestCheckpoint(cancellationToken);
    return checkpoint.WaitForSync(cancellationToken);
}
```
</CodeGroup>
...
try
{
    ...
    await checkpoint.WaitForSync().WaitAsync(TimeSpan.FromSeconds(3));
}
catch (TimeoutException)
{
    ShowRefreshMessage("The refresh timed out. Try again.");
}
catch (CheckpointRequestException e) when (e.Message == Disconnected)
{
    ShowRefreshMessage("Reconnect before refreshing again.");
}
...
- For .NET, implement `ICustomCheckpointRequestConnector`.
...
The .NET SDK also represents requests as strings for compatibility with older SDK APIs; this will change in a future release
of the .NET SDK.
</Tip>
