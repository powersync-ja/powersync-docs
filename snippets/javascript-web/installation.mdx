Add the [PowerSync Web NPM package](https://www.npmjs.com/package/@powersync/web) to your project:

<Tabs>
  <Tab title="npm">
    ```bash
    npm install @powersync/web
    ```
  </Tab>

  <Tab title="yarn">
    ```bash
    yarn add @powersync/web
    ```
  </Tab>

  <Tab title="pnpm">
    ```bash
    pnpm install @powersync/web
    ```
  </Tab>
</Tabs>

**Required peer dependencies**
  
This SDK currently requires [`@journeyapps/wa-sqlite`](https://www.npmjs.com/package/@journeyapps/wa-sqlite) as a peer dependency. Install it in your app with:
  
<Tabs>
  <Tab title="npm">
    ```bash
    npm install @journeyapps/wa-sqlite
    ```
  </Tab>

  <Tab title="yarn">
    ```bash
    yarn add @journeyapps/wa-sqlite
    ```
  </Tab>

  <Tab title="pnpm">
    ```bash
    pnpm install @journeyapps/wa-sqlite
    ```
  </Tab>
</Tabs>

<Info>
  By default, this SDK connects to a PowerSync instance via WebSocket (from `@powersync/web@1.6.0`) or HTTP stream (before `@powersync/web@1.6.0`). See [Developer Notes](/client-sdk-references/javascript-web#developer-notes) for more details on connection methods.
</Info> 