...
* **Reads vs Writes**: PowerSync handles syncing of reads differently from writes (mutations)
   * **Reads**: The PowerSync Service connects to your MongoDB database for real-time replication of data, and syncs data to clients based on the [Sync Streams](/sync/streams/overview) configuration (or legacy [Sync Rules](/sync/rules/overview)). Sync Streams/Rules are more flexible than MongoDB Realm Flexible Sync, but are defined on the server-side, not on the client-side.
