...
class StatusIndicator
{
    ...
    {
        _db = db;
        _currentStatus = db.CurrentStatus;
        ...
        Task.Run(async () =>
        {
            while (true)
            {
                var newStatus = _db.CurrentStatus;
...
const int priority = 1; // Your bucket priority
await db.WaitForFirstSync(priority: priority);
```
...
## Monitor Priority Status Entries
