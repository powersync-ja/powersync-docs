...
<warning>
Heads up: we’ve introduced a new attachments library at `package:powersync_core/attachments/attachments.dart`. It has cleaner APIs and is where we'll publish new features. This guide still uses `powersync_attachments_helper`, but if you’re starting a new project, consider using the new library. Read more here: [powersync_core/attachments](https://github.com/powersync-ja/powersync.dart/tree/main/packages/powersync_core/doc/attachments.md).
</warning>
