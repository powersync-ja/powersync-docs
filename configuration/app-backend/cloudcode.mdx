...
<Steps>
  ...
  <Step title="Request an egress token from the PowerSync support team.">
    Request a egress token from the PowerSync support team by sending an email to support@powersync.com or by visiting the [PowerSync Support](https://support.powersync.com/) website.

    Please include the following in the request:

    * Subject: PowerSync CloudCode Static IP Address Request
    * Body: I've completed step 1 of configuring static IP addresses for CloudCode to connect to the MongoDB database. Can you please provide me with an egress token? Here is the MongoDB replica's connection/port uri: `<uri_1,uri_2,etc.>`

    The PowerSync support team will respond with an egress token. Keep record of the egress token and use it in the next step. Add the egress token to the environment variables of the deployment. The environment variable name should be `CLOUDCODE_EGRESS_TOKEN`.
  </Step>
  <Step title="Configure the IP address in the MongoDB Atlas cluster.">
    ...
    Here's a list of the IP addresses that are allowed to connect to the MongoDB database:
    ```javascript
    {
        eu: {
            A: ['52.17.49.128', '52.18.47.75'],
            B: ['52.50.132.241', '52.209.110.101'],
            C: ['52.214.251.96', '54.229.199.114']
        },
        us: {
            A: ['18.210.190.20', '34.192.9.106'],
            B: ['18.211.143.94', '34.193.213.9'],
            C: ['54.210.43.215', '52.201.67.23']
        }
    }
    ```
  </Step>
