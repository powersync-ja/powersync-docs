...
Your app reads and writes directly to its local database, so user interactions feel instant and your app will continue to work even when offline. The local database is synced, so changes appear across users and devices in real-time.
