...
description: "Keep your UI current with basic, incremental, and differential watch queries over client-side SQLite."
---
...
Watch queries turn a SQL query into a subscription. When a local write or incoming sync changes a dependent table, PowerSync reruns the query against SQLite and returns the latest results to your app. Your UI can react to changes without polling, manual refetching, or separate cache invalidation logic, and these changes can arrive from the local device or from other users and devices.

Prefer not to write SQL directly? See [ORM support](/client-sdks/orms/js/overview) for type-safe query builders and ORM integrations. Reactive-query features vary by SDK and ORM.
