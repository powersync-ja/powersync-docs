...
<Tab title="Kotlin Compose">
  The `composeSyncStream` extension composeSyncStream subscribes to a stream when the composable enters the composition and automatically unsubscribes when it is disposed. It returns `SyncStreamStatus?` for the subscription so you can check sync state:
  ```kotlin
  @Composable
  fun TodoList(listId: String) {
    val database = LocalPowerSync.current
