...
|--------|---------|-------------|
| `query` | — | SQL-like query defining which data to sync. Use either `query` or `queries`, not both. See [Supported SQL](/sync/supported-sql) |
| `queries` | — | Array of queries defining which data to sync. Data from all the queries will populate the same [bucket](/architecture/powersync-service#bucket-system). See [Multiple Queries](/sync/streams/queries#multiple-queries-per-stream). |
