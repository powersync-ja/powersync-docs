...
description: Reuse common query patterns within or across streams using CTEs to simplify configurations and improve efficiency.
---
...
**Global CTE names must not shadow source table names.** If a global CTE has the same name as a database table or collection, PowerSync reports a validation error. This prevents hard-to-debug situations where a global CTE silently changes the behavior of a stream defined elsewhere in the config. Stream-level CTE names are not subject to this restriction — within a stream, a local CTE always takes precedence over any table of the same name.
