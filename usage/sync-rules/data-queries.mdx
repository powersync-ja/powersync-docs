---
title: "Data Queries"
description: "Data queries select the data that form part of a bucket, using the bucket parameters."
---
...
<Info>
  MongoDB uses `_id` as the name of the ID field in collections. Therefore, PowerSync requires using `SELECT _id as id` in the data queries when [using MongoDB](/usage/installation/database-setup) as the backend source database.
</Info>
