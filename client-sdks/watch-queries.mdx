...
description: 'Build reactive UIs with watch queries that update when data changes'
---

import JavaScriptWatch from '/snippets/javascript/basic-watch-query.mdx';
...
PowerSync supports basic watch queries across all SDKs. These APIs return query results whenever the underlying tables change.
...
## WatchedQuery (Recommended)

The `WatchedQuery` class includes loading, fetching and error states, supports multiple listeners, automatic cleanup on PowerSync close, and the `updateSettings()` API for dynamic parameter changes. This is the preferred approach for JavaScript SDKs.

<CodeGroup>
  ```javascript WatchedQuery (Default)
  ...
  ```javascript WatchedQuery (With Comparator)
  // WatchedQuery with configurable comparator that compares result sets
  // before emitting to listeners, preventing unnecessary listener
  // invocations when data hasn't changed.
  const pendingLists = db
    .query({
      sql: 'SELECT * FROM lists WHERE state = ?',
      parameters: ['pending']
...
React hooks that preserve object references for unchanged items and use row-level comparators to minimize re-renders. Use these when you want built-in state management plus incremental updates for React components:
...
## Legacy APIs (with Comparator)

The existing `db.watch()` APIs also support incremental updates via a `comparator` option:
