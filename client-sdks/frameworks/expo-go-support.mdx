...
description: "Run PowerSync in Expo Go without native modules using @powersync/adapter-sql-js."
sidebarTitle: "Expo Go"
---
...
1. [**Define what data to sync by setting up Sync Streams**](/sync/streams/overview) 
...
<Note>
  OP-SQLite cannot run in Expo Go because it requires native code compilation. Specifically, PowerSync needs a SQLite implementation that can load the PowerSync SQLite core extension, which isn't possible in Expo Go's prebuilt app container. 
</Note>

This adapter provides better performance, full SQLite consistency guarantees, and is suitable for both development builds and production deployment. See the SDKs [Installation](/client-sdks/reference/react-native-and-expo#installation) details for setup instructions.
