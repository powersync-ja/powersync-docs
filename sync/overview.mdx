...
- **On-demand syncing**: You define Sync Streams on the PowerSync Service, and a client can then subscribe to them one or more times with different parameters, on-demand. You still have the option of auto-subscribing certain buckets when a client connects for "sync data upfront" behavior.
- **Temporary caching-like behavior**: Each subscription includes a configurable TTL that keeps data active after the client unsubscribes, acting as a warm cache for recently accessed data.
...
<Card title="Sync Streams (Early Alpha)" horizontal icon="flask-gear" href="/sync/streams/overview">
</Card>
...
- **In the case of Sync Rules:** Definitions of the different _buckets_ that exist (basically logical groupings of data), with SQL-like queries to specify the parameters used by each bucket, as well as the data contained in each bucket.
...
<Note>
A _parameter_ is a value that can be used in the Sync Rules or Streams to create dynamic sync behavior for each user/client. Each client syncs only the relevant [_buckets_](#sync-rules/streams-determine-replication-from-the-source-database) based on the parameters for that client.
...
* In the _Sync Rules_ system, buckets and their parameters are explicitly defined.
...
There are limitations on the SQL syntax and functionality that is supported in the Sync Rules and Sync Streams. For details and limitations, see [Operators and Functions](/sync/operators-and-functions).
...
A PowerSync Service instance replicates and transforms relevant data from the backend source database according to the deployed Sync Rules or Sync Streams. During replication, data and metadata is persisted in [buckets](/architecture/powersync-service#bucket-system) on the PowerSync Service. Buckets are incrementally updated so that they contain the latest state as well as a history of changes (operations). This is key to how PowerSync achieves efficient delta syncing — having the operation history for each bucket allows clients to sync only the deltas that they need to get up to date.

<Note>
As a practical example, let's say you have a bucket named `user_todo_lists` that contains the to-do lists for a user, and that bucket utilizes a `user_id` parameter (which will be embedded in the JWT). Now let's say users with IDs `A` and `B` exist in the source database. PowerSync will then replicate data from the source database and create individual buckets with bucket IDs `user_todo_lists["A"]` and `user_todo_lists["B"]`.
</Note>
...
<Frame caption="Operations in buckets are synced to clients in real-time based on Sync Rules/Streams, and a client-side SQLite database is materialized based on the data in the buckets.">
