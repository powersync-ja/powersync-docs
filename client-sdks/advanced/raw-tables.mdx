...
To reduce this downtime, the Kotlin and Swift SDKs support a soft clear: pass `soft = true` (Kotlin) or
`soft: true` (Swift) to `disconnectAndClear()`. A soft clear runs the same raw-table `clear` statements
