...
description: "Build Supabase apps with PowerSync that use client-side SQLite for real-time updates across users and devices, instant interactions and continued functionality when offline."
...
[Sync Streams](/sync/streams/overview) allow developers to control which data gets synced to which user devices using a SQL-like syntax in a YAML file. For the demo app, we're going to specify that each user can only see their own to-do lists and list items.

1. In the [PowerSync Dashboard](https://dashboard.powersync.com/), select your project and instance and go to the **Sync Streams** view.
