...
You can show users a progress bar when data downloads using the `syncStatus.downloadProgress` property. This is especially useful for long-running initial syncs. `downloadProgress.fraction` gives a value from 0.0 to 1.0 representing the total sync progress. 
