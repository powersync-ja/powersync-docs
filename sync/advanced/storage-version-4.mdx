...
After enabling S3 object storage, you can raise [`max_concurrent_connections`](/configuration/powersync-service/self-hosted-instances#param-max-concurrent-connections) from its default of 200 per API process. More concurrent connections increase CPU and memory usage. Before applying a higher limit in production, test it with the data volumes and concurrent initial syncs you expect, and check CPU usage, memory usage, and sync times.
