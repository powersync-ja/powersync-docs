...
<Warning>
  * Full multi-tab support relies on shared web workers, which are disabled by default on Android, iOS, and Safari. On these platforms, the SDK falls back to sharing sync status across tabs, as described below.
...
When multi-tab support is disabled, whether explicitly or because the platform does not support it, each tab spawns a standard web worker for database operations. These workers can safely operate on the database concurrently, but changes made in one tab may not update watched queries in other tabs. Only one tab connects and syncs at a time, and only that tab's `fetchCredentials` and `uploadData` methods are called.

Since version 2.1.0 of the SDK, the syncing tab shares its sync status with other tabs over a broadcast channel. Fields like `hasSynced` and download progress update in every tab, and sync stream subscriptions made in any tab are included in the active sync connection. This provides limited multi-tab support on platforms where shared workers are unavailable.
...
When disabled, each tab uses independent workers. Database changes made in one tab do not automatically update watched queries in other tabs, but the syncing tab still shares its sync status across tabs (since version 2.1.0 of the SDK).
