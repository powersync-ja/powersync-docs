...
containing only an `id` (TEXT) and `data` (JSON) column.
...
When opting in to raw tables, you are responsible for creating the tables before using them - PowerSync will no longer
create them automatically.

Because PowerSync takes no control over raw tables, you need to manually:

1. Tell PowerSync how to map the [schemaless protocol](/architecture/powersync-protocol#protocol) into your existing tables.
...
To sync into the existing `todo_lists` table instead of `ps_data__`, PowerSync needs the SQL statements extracting
...
The PowerSync client as part of our SDKs will automatically run these statements in response to sync lines being sent from
the PowerSync service.

To reference the ID or extract values, prepared statements with parameters are used. `delete` statements can reference the id
of the affected row, while `put` statements can also reference individual column values.
...
<Warning>Also note that raw tables are only supported by the new Rust sync client, which is currently opt-in.</Warning>
...
insertions, updates and deletions automatically forwards local writes into this table.
...
The virtual table associates writes with the current transaction and ensures writes made during the sync
