...
<Tabs>
      ...
      <Tab title=".NET">
		Use [SyncStatus](https://github.com/powersync-ja/powersync-dotnet/blob/main/PowerSync/PowerSync.Common/DB/Crud/SyncStatus.cs) and [db.Events.OnStatusChanged.ListenAsync](https://github.com/powersync-ja/powersync-dotnet/blob/main/PowerSync/PowerSync.Common/CHANGELOG.md) (since v0.0.11-alpha.1) to listen for status changes to your PowerSync instance. Start the listener when your app connects (e.g. `_ = statusIndicator.StartListeningAsync(db, cts.Token)`).
