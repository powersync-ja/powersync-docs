...
## The `waitForFirstSync(priority: int)` method to allow waiting for a specific priority level.

TODO: add example

## The `SyncStatus.statusInPriority(priority: int)` property providing details on the last sync time per priority level.
