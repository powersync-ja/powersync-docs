...
<AccordionGroup>
    ...
    <Accordion title="Step 2: Create Supabase Edge Functions">
        ...
        Before we create the Edge Functions, we need to set up the environment variables for the AWS S3 credentials. Create a `.env` file in the root of your Supabase project and add the following:
        ```bash .env
        AWS_ACCESS_KEY_ID=***
        AWS_SECRET_ACCESS_KEY=***
        ```

        For more information on getting started with Supabase Edge Function, see their [Getting Started Guide](https://supabase.com/docs/guides/functions/quickstart).
        <Accordion title={"Upload Edge Function"}>
            ...
            Deno.serve(async (req) => {
                ...
                if (!S3Region || !S3BucketName || !fileName || !mediaType) {
                    return new Response(
                        JSON.stringify({ error: 'Missing required fields: S3Region, S3BucketName, fileName, or mediaType' }),
                        { status: 400 }
                    );
                }
                try {
        ...
        <Accordion title={"Download Edge Function"}>
            ...
            Deno.serve(async (req) => {
                ...
                const { S3Region, S3BucketName, fileName, expiresIn } = await req.json();
                if (!S3Region || !S3BucketName || !fileName ) {
                    return new Response(
                        JSON.stringify({ error: 'Missing required fields: S3Region, S3BucketName or fileName' }),
                        { status: 400 }
                    );
                }
                try {
        ...
        <Accordion title={"Delete Edge Function"}>
            ...
            Deno.serve(async (req) => {
                ...
                const { S3Region, S3BucketName, fileName } = await req.json();
                if (!S3Region || !S3BucketName || !fileName ) {
                    return new Response(
                        JSON.stringify({ error: 'Missing required fields: S3Region, S3BucketName or fileName' }),
                        { status: 400 }
                ...
                try {
                    ...
                    return new Response(JSON.stringify({ message: `${fileName} deleted successfully from ${S3BucketName}.` }), {
                        headers: { "Content-Type": "application/json" },
                        status: 200,
                    });
                } catch (err) {
                    return new Response(JSON.stringify({ error: `Error deleting ${fileName} from ${S3BucketName}: ${err}`}), {
                        headers: { "Content-Type": "application/json" },
                        status: 500,
                    });
                }
    ...
    <Accordion title="Step 3: Update app configuration" defaultOpen={false}>
        <Steps>
            <Step title="Update process-env.d.tss">
                ...
                ```typescript process-env.d.ts {12-13}
                export {};
            ...
            <Step title="Update AppConfig.ts">
                ...
                ```typescript AppConfig.ts {6-7}
                export const AppConfig = {
                    supabaseUrl: process.env.EXPO_PUBLIC_SUPABASE_URL,
                    supabaseAnonKey: process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY,
                    supabaseBucket: process.env.EXPO_PUBLIC_SUPABASE_BUCKET || '',
                    powersyncUrl: process.env.EXPO_PUBLIC_POWERSYNC_URL,
                    awsS3Region: process.env.EXPO_PUBLIC_AWS_S3_REGION,
                    awsS3BucketName: process.env.EXPO_PUBLIC_AWS_S3_BUCKET_NAME || ''
                };
                ```
            </Step>
        </Steps>
    ...
    <Accordion title="Step 4: Add AWS Storage Adapter">
        ...
        export class AWSStorageAdapter implements StorageAdapter {
            ...
            async uploadFile(
                ...
                const response = await this.client.functions.invoke<S3Upload>('s3-upload', {
                    body: {
                        S3Region: AppConfig.awsS3Region,
                        S3BucketName: AppConfig.awsS3BucketName,
                        fileName: filename,
                ...
                try {
                    ...
                    console.log(`File uploaded successfully to ${AppConfig.awsS3BucketName}/${filename}`);
                } catch (error) {
            ...
            async deleteFile(uri: string, options?: { filename?: string }): Promise<void> {
                ...
                try {
                    ...
                    console.error(`Error deleting ${filename} from ${AppConfig.awsS3BucketName}:`, error);
                }
            }
    ...
    <Accordion title="Step 5: Use AWS Storage Adapter">
        ...
        ```typescript system.ts {12, 18, 26, 46}
    ...
    <Accordion title="Step 6: Run the app">
        <Note>
            Ensure that `AppConfig.supabaseBucket` is replaced with `AppConfig.awsS3BucketName` in the [React Native To-Do List demo app](https://github.com/powersync-ja/powersync-js/tree/main/demos/react-native-supabase-todolist) in both
            1. [TodoItemWidth.ts](https://github.com/powersync-ja/powersync-js/blob/main/demos/react-native-supabase-todolist/library/widgets/TodoItemWidget.tsx#L86) and
            2. [PhotoAttachmentQueue.ts](https://github.com/powersync-ja/powersync-js/blob/main/demos/react-native-supabase-todolist/library/powersync/PhotoAttachmentQueue.ts#L9)
        </Note>
